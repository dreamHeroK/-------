...
-- @Last Modified time: 2023-12-17 10:43:19
local sj = 取随机数
	...
	if 内容[5] then
  	...
  	if 名称=="老马猴" then
  		if 事件=="我想认证法宠" then
  			...
  			-- 	if 玩家数据[数字id].召唤兽.数据[i] then
  			-- 	    xx[#xx+1]=玩家数据[数字id].召唤兽.数据[i].名称
  			-- 	    jc[i]={名称=玩家数据[数字id].召唤兽.数据[i].名称,宝宝序号=i,认证码=玩家数据[数字id].召唤兽.数据[i].认证码}
  			-- 	end
  			-- end
  			-- jc.类型="法术认证"
  			-- xx[#xx+1]="取消"
  			-- 添加最后对话(数字id,"不是每个召唤兽都可以认证法宠的哦，我想你应该知道认证法宠需要什么条件了吧！你要选择哪知召唤兽来认证“法宠”呢？",xx,jc)
...
-- elseif 返回信息~=nil and 事件~="取消" and 事件~="我再考虑考虑" then
-- 			if 返回信息.类型=="法术认证" then
-- 				local bb = {}
-- 				local 认证技能={"上古灵符","水攻","落岩","雷击","烈火","地狱烈火","奔雷咒","水漫金山","泰山压顶"}
-- 				local 找到技能={}
-- 			    for i=1,#返回信息 do
-- 			    	if 事件==返回信息[i].名称 then
-- 			    		bb=返回信息[i]
-- 			    	    break
-- 			    	end
-- 			    end
-- 			    if 玩家数据[数字id].召唤兽.数据[bb["宝宝序号"]] then
-- 			    	if 玩家数据[数字id].召唤兽.数据[bb["宝宝序号"]].法术认证~=nil then
-- 			    		添加最后对话(数字id,"你的召唤兽已经认证过“法宠”了")
-- 					    return
-- 					end
-- 			    	for n=1,#玩家数据[数字id].召唤兽.数据[bb["宝宝序号"]].技能 do
-- 						for i=1,#认证技能 do
-- 							if 认证技能[i]==玩家数据[数字id].召唤兽.数据[bb["宝宝序号"]].技能[n] then
-- 								找到技能[#找到技能+1]=玩家数据[数字id].召唤兽.数据[bb["宝宝序号"]].技能[n]
-- 							end
-- 						end
-- 					end
-- 			    end
...
-- 			        return
-- 			    end
-- 			    local 等级=玩家数据[数字id].召唤兽.数据[bb["宝宝序号"]].参战等级
-- 			    local 金钱=等级*等级*160+等级*4000
-- 			    if 玩家数据[数字id].召唤兽.数据[bb["宝宝序号"]].种类=="神兽" then
-- 			        金钱=1000000
-- 			    end
-- 			    bb.类型="选择认证技能"
-- 			    local 对话="我身子骨比较单薄，其他活已经干不动了#74只好靠这个点绝活来赚点小钱呢#17帮你认证这只召唤兽需要#Y"..金钱.."#W两银子，你要认证吗#55"
-- 			    找到技能[#找到技能+1]="我再考虑考虑"
-- 			    添加最后对话(数字id,对话,找到技能,bb)
-- 		    elseif 返回信息.类型=="选择认证技能" then
-- 				local 等级=玩家数据[数字id].召唤兽.数据[返回信息["宝宝序号"]].参战等级
-- 				if #玩家数据[数字id].召唤兽.数据[返回信息["宝宝序号"]].技能<2 then --为了防止打书循环卡死，小于2技能的宝宝不得认证
-- 					添加最后对话(数字id,"你的召唤兽携带技能小于2，无法认证！")
-- 				    return
-- 				end
-- 			    local 金钱=等级*等级*160+等级*4000
-- 			    if 玩家数据[数字id].召唤兽.数据[返回信息["宝宝序号"]].种类=="神兽" then
-- 			        金钱=5000000
-- 			    end

-- 		        local 认证技能={"上古灵符","水攻","落岩","雷击","烈火","地狱烈火","奔雷咒","水漫金山","泰山压顶"}
-- 		        local go =false
-- 		        if 玩家数据[数字id].召唤兽.数据[返回信息["宝宝序号"]] then
-- 			    	for n=1,#玩家数据[数字id].召唤兽.数据[返回信息["宝宝序号"]].技能 do
-- 						for i=1,#认证技能 do
-- 							if 认证技能[i]==事件 and 认证技能[i]==玩家数据[数字id].召唤兽.数据[返回信息["宝宝序号"]].技能[n] then
-- 								go = true
-- 								break
-- 							end
-- 						end
...
-- 							local 重复=false
-- 							for n=1,#玩家数据[数字id].召唤兽.数据[返回信息["宝宝序号"]].技能 do
-- 								if 玩家数据[数字id].召唤兽.数据[返回信息["宝宝序号"]].技能[n]==认证技能[i] then
-- 									重复=true
-- 								end
-- 							end
-- 							if 重复==false then
...
-- 							end
-- 						end
-- 						if #添加技能>0 and #玩家数据[数字id].召唤兽.数据[返回信息["宝宝序号"]].技能<24 then
-- 							local 新技能=添加技能[取随机数(1,#添加技能)]
-- 							玩家数据[数字id].召唤兽.数据[返回信息["宝宝序号"]].技能[#玩家数据[数字id].召唤兽.数据[返回信息["宝宝序号"]].技能+1]=新技能
-- 							常规提示(数字id,"法术认证成功！你的召唤兽获得了新技能#G"..新技能)
-- 							玩家数据[数字id].召唤兽.数据[返回信息["宝宝序号"]].法术认证=事件
-- 							玩家数据[数字id].召唤兽.数据[返回信息["宝宝序号"]]:刷新信息()
-- 							发送数据(连接id,16,玩家数据[数字id].召唤兽.数据)--玩家数据[数字id].召唤兽.数据[返回信息["宝宝序号"]]
-- 						end
-- 			    	end
-- 			    else
