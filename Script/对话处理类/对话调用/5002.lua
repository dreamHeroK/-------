...
-- @最后修改来自: baidwwy
-- @Last Modified time: 2025-08-25 13:41:02
local sj = 取随机数
...
__NPCdh111[5002]=function (ID,编号,页数,已经在任务中,数字id)
    ...
    local 临时名称=取假人表(5002)
      ...
      local 神兜兜数量 = 99
      local 灵兜兜数量 = 1
      if 临时名称[编号].名称 == "超级鲲鹏" then
        神兜兜数量 = 2999
        灵兜兜数量 = 10
        if 玩家数据[数字id].角色.神兽兑换 == "超级鲲鹏" then
          wb[1] = "我是“"..临时名称[编号].名称.."使者”，如果你身上有"..神兜兜数量.."个神兜兜和"..灵兜兜数量.."个灵兜兜可以在我这里领取神兽“"..临时名称[编号].名称.."”！当然您也可以看看我或者来了解一下我们的族群，我最喜欢聊天了。"
          xx = {"我来带你一起回家（领取时需要花费50点体力）","我还有事，下回再来好好交流吧"}
        else
          wb = {}
          xx = {}
        end
...
__NPCdh222[5002]=function  (id,数字id,序号,内容)
    ...
    if 事件=="我来带你一起回家（领取时需要花费50点体力）" then
      添加最后对话(数字id,"少侠你要选择哪种类型的"..名称,{"物理型（单点爆伤）","法术型（秒伤）","让我思考一下"})
