	...
	local 事件=内容[1]
  	...
  	if 名称=="超级神柚" then
	      if 事件=="我要挑战你" then
	      	...
	      	if __副本每日.会员地图[数字id]==nil or __副本每日.会员地图[数字id].日期~=今日日期  then--"#Y
                		__副本每日.会员地图[数字id]={神柚=0,日期=今日日期}
                		end
...
for i=1,#id组 do
	...
	local 今日日期 = tonumber(os.date("%Y%m%d"))
	if __副本每日.会员地图[id]==nil or __副本每日.会员地图[id].日期~=今日日期  then--"#Y
	     __副本每日.会员地图[id]={神柚=0,日期=今日日期}
	end
