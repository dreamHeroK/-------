-- @作者: baidwwy
-- @邮箱:  313738139@qq.com
-- @创建时间:   2021-01-31 11:28:58
-- @最后修改来自: baidwwy
-- @Last Modified time: 2025-12-05 11:04:47
local sj = 取随机数
...
__NPCdh111[1501]=function (ID,编号,页数,已经在任务中,数字id)
	...
	local xx = {}
	if 编号 == 1 then
		xx = {"我要更换现在的宠物","我来兑换圣兽丹","我要兑换进阶召唤兽的饰品","我想更换召唤兽装饰品的颜色","我要切换进阶召唤兽的造型","我要自定义宝宝造型","我什么都不想做"}
		return{"普陀山_接引仙女","宠物仙子","你可以在这里更换携带的宠物，宠物可以在战斗中捕捉召唤兽和控制多只召唤兽为你作战。",xx}
	elseif 编号 == 2 then
	...
	elseif 编号 == 15 then
		...
		local xx = {"我的召唤兽受伤了，请帮我救治一下吧","我的召唤兽忠诚度降低了，请帮我驯养一下吧","我要同时补满召唤兽的气血、魔法和忠诚","我要提升召唤兽忠诚","我只是看看","我要重置召唤兽属性点","我要重置召唤兽等级"}
...
__NPCdh222[1501]=function  (id,数字id,序号,内容)
	...
	if 名称=="宠物仙子" then
		...
		elseif 事件=="我要切换进阶召唤兽的造型" then
			...
			if 玩家数据[数字id].角色.参战宝宝.进阶 and 玩家数据[数字id].角色.参战宝宝.进阶.灵性>=50 then
				添加最后对话(数字id,"切换宝宝造型需要支付3000000两银子作为手续费，少侠可要考虑好了。",{"我想好了，帮我切换吧！","取消"})
			else
			    添加最后对话(数字id,"少侠您的召唤兽好像无法转换造型。")
			end
		elseif 事件=="我要自定义宝宝造型" then
			添加最后对话(数字id,"自定义宝宝造型仅 神兽 才可以操作，每次需支付30000000两银子作为手续费。",{"我想好了，开始吧！","取消"})
   ...
   --   			if 玩家数据[数字id].角色.参战信息==nil then
			...
			-- end
			-- local 编号=玩家数据[数字id].召唤兽:取编号(玩家数据[数字id].角色.参战宝宝.认证码)
			-- if 编号~=0 and 玩家数据[数字id].召唤兽.数据[编号] and string.find(玩家数据[数字id].召唤兽.数据[编号].模型,"锦衣")~=nil then
			-- 	添加最后对话(数字id,"已经激活锦衣的宠物，无法改变造型了#3")
			--   return
			-- end
			-- 发送数据(id,68)
		...
		elseif 事件=="我想好了，帮我切换吧！" then
			...
			if 编号~=0 and 玩家数据[数字id].召唤兽.数据[编号] and 玩家数据[数字id].召唤兽.数据[编号].进阶 and 玩家数据[数字id].角色.参战宝宝.进阶.灵性>=50 then
				...
				if string.find(玩家数据[数字id].召唤兽.数据[编号].模型,"进阶")~=nil then
					if 玩家数据[数字id].角色:扣除银子(3000000,0,0,"切换宝宝造型") then
					  玩家数据[数字id].召唤兽.数据[编号].模型=string.gsub(玩家数据[数字id].召唤兽.数据[编号].模型,"进阶","")
					  发送数据(id,6556,玩家数据[数字id].召唤兽.数据[编号])
						添加最后对话(数字id,"恭喜你的召唤兽获得了新的形态！")
					end
				else
					local mx1="锦衣"..玩家数据[数字id].召唤兽.数据[编号].模型
				          	local go=false
				          	if 糖果专用系统:取召唤兽锦衣(mx1) then
				              go=false
				          	end
     ...
     elseif 名称=="勾魂马面" then
     	  if 事件=="我已经想清楚了" then
     	  	local ggg = xxx自杀[取随机数(1,#xxx自杀)]
     	  	local xxx = "#Y/沙雕！自杀要想清楚，机会没第二次！\n回答对问题立刻杀，" ..ggg[1] .."+" ..ggg[2] .."等于多少？"
     	  	local daan = {}
     	  	for an=1,5 do
     	  		local sddd = 取随机数(3,88)
     	  		daan[an] = "" ..sddd
     	  	end
     	  	daan[取随机数(1,5)] = "" ..ggg[3]
     	  	玩家数据[数字id].zsda = "" ..ggg[3]
     	  	添加最后对话(数字id,xxx,daan)
     	  elseif 玩家数据[数字id].zsda then
     	  	if 事件 == 玩家数据[数字id].zsda then
	     	  	玩家数据[数字id].角色:删除角色(数字id)
	     	  else
	     	  	玩家数据[数字id].zsda = nil
	     	  	常规提示(数字id,"#Y/沙雕！这么简单的问题都不会！去死吧...")
	     	  end
     	  end
    ...
    elseif 名称=="梨园小贩" then
        if 事件=="别废话，我是来买票的" then
        	-- 五更寒=200
        	local 积分1,积分2,积分3 = 150,200,150
        	if 正式 then 积分1,积分2,积分3 = 150,200,150 end
        	发送数据(id,304,{积分=玩家数据[数字id].角色.副本积分,价格={积分1,积分2,积分3}})
        end
    elseif 名称=="癫散班主" then
