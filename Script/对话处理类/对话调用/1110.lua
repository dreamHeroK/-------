-- @作者: baidwwy
-- @邮箱:  313738139@qq.com
-- @创建时间:   2021-01-31 18:39:20
-- @最后修改来自: baidwwy
-- @Last Modified time: 2025-08-24 01:09:31
local sj = 取随机数
...
__NPCdh111[1110]=function (ID,编号,页数,已经在任务中,数字id)
  ...
  elseif 编号==9 then
    ...
    elseif 编号==15 then
        wb[1] = "你要进去看望朋友呀?我送你进去,不要惹事就好。"
        local xx = {"请送我前往青丘","开启青丘迷雾副本","我还要逛逛"}
        if 玩家数据[数字id].角色:取任务(410)~=0 then
      wb[1]="你的副本已经开启了，是否需要我帮你传送进去？"
      xx={"请送我去青丘","我等会再进去"}
     end
        return {"进阶花铃","花铃",wb[取随机数(1,#wb)],xx}
...
__NPCdh222[1110 ]=function  (id,数字id,序号,内容)
  ...
  elseif 名称=="接引守卫" then
   ...
   elseif 名称=="花铃" then
    if 事件=="开启青丘迷雾副本" then
      副本_青丘迷雾:开启副本(数字id)
      elseif 事件=="请送我去青丘" then
      任务处理类:副本传送(数字id,41)
      elseif 事件=="请送我前往青丘" then
        if 时辰信息.昼夜 == 1 then --晚上
        地图处理类:跳转地图(数字id,1375,111,190)
      else
        地图处理类:跳转地图(数字id,1284,111,190)
      end
    end
