...
-- @Last Modified time: 2024-01-07 13:25:44
-- @作者: baidwwy
-- @邮箱:  313738139@qq.com
...
__NPCdh111[1208]=function (ID,编号,页数,已经在任务中,数字id)
	...
	if 编号==1 then
			...
			if f函数.文件是否存在([[data/]]..玩家数据[数字id].角色.账号)==false then  --未创建存档
		        return
		    end
		    local 账号信息 =f函数.读配置(程序目录..[[data\]]..玩家数据[数字id].角色.账号..[[\账号信息.txt]],"账号配置","管理")+0
			if 账号信息==2024 then
				if 玩家数据[数字id].管理模式 then
					xx={"打开管理器","设置物品出售表"}
				end
			end
