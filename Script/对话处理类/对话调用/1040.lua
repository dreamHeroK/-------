...
-- @Last Modified time: 2025-10-27 15:32:05
-- @Author: baidwwy
-- @Date:   2023-10-25 19:03:14
-- @Last Modified by:   baidwwy
-- @Last Modified time: 2025-03-10 11:47:00
-- @作者: baidwwy
-- @邮箱:  313738139@qq.com
-- @创建时间:   2021-01-31 18:49:32
-- @最后修改来自: baidwwy
...
__NPCdh111[1040]=function (ID,编号,页数,已经在任务中,数字id)
  ...
  elseif 编号==9 then
    ...
    xx={"好的快送我一程。","婆婆！我还没准备好。"}
    if 玩家数据[数字id].角色:取任务(980)~=0 then
    ...
    elseif 编号 == 11 then
    wb[1] = "岁月流转，多少人遗忘了自己的梦想，又有多少人还因为梦想而仍在努力，少侠若是有心，能否帮小仙几个小忙?(每日可做2轮)"
    local xx = {"请吩咐","我想取消任务","我只是看看"}
      return {"普陀_接引仙女","幻梦仙子",wb[取随机数(1, #wb)],xx}
  ...
  local 名称=内容[3]
    ...
    elseif 事件=="藏宝阁物品和银子取回" then
          ...
          if 寄存数据[id] == nil then
            ...
            elseif 寄存数据[id][1].类型 == "仙玉" then
              添加仙玉(id,玩家数据[id].账号, 寄存数据[id][1].数额, "藏宝阁出售寄存")
              table.remove(寄存数据[id],1)
              local 剩余 = 0
              if #寄存数据[id] > 0 then
                剩余 = #寄存数据[id]
              ...
              发送数据(玩家数据[id].连接id,7,"#y/领取寄存物品成功,当前剩余"..剩余.."份待领取")
            elseif 寄存数据[id][1].类型 == "点卡" then
              添加点卡(id,玩家数据[id].账号, 寄存数据[id][1].数额, "藏宝阁出售寄存")
              table.remove(寄存数据[id],1)
              local 剩余 = 0
              if #寄存数据[id] > 0 then
                剩余 = #寄存数据[id]
