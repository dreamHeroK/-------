...
-- @Last Modified time: 2025-09-30 17:42:26
				...
				if 战斗准备类.战斗盒子[战斗id] then --估计是清空重复了，所以，客户端会闪退
					...
					for n=1,#参战玩家 do
						if 玩家数据[参战玩家[n].id]~=nil then
							...
							发送数据(玩家数据[参战玩家[n].id].连接id, 20.1, {气血=玩家数据[参战玩家[n].id].角色.参战宝宝.最大气血,魔法=玩家数据[参战玩家[n].id].角色.参战宝宝.最大魔法})
							...
							if czdw[i] and czdw[i].类型=="角色" and 玩家数据[czdw[i].玩家id] then
								...
								for n, v in pairs(玩家数据[数字id].角色.装备) do --这个是穿戴的装备
									...
									if 玩家数据[数字id].道具.数据 and 玩家数据[数字id].道具.数据[格子] ~= nil then
										...
										local 不磨存在 = false
										if 玩家数据[数字id].道具.数据[格子].特效 then
											for i,v in ipairs(玩家数据[数字id].道具.数据[格子].特效) do
												if v == "永不磨损" then
													不磨存在 = true
													break
												end
										...
										if 玩家数据[数字id].道具.数据[格子].耐久 ~= nil and not 不磨存在 then
											...
											if n==3 then
												...
												if czdw[i].耐久计算 and czdw[i].耐久计算.施法>0 then
													玩家数据[数字id].道具.数据[格子].耐久=玩家数据[数字id].道具.数据[格子].耐久-czdw[i].耐久计算.施法/20
												end
											else
												if czdw[i].耐久计算 and czdw[i].耐久计算.挨打>0 then
								...
								for k,v in pairs(玩家数据[数字id].角色.灵饰) do
									...
									if 玩家数据[数字id].道具.数据 and 玩家数据[数字id].道具.数据[v] ~= nil then
										...
										if 玩家数据[数字id].道具.数据[v].耐久 and 玩家数据[数字id].道具.数据[v].耐久<=0 then
