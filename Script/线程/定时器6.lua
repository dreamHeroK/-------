...
-- @Last Modified time: 2025-10-22 00:31:51
	...
	self.线程.消息返回 = function(self,...)
		...
		elseif ... == "循环更新" then
				    	...
				    	if 玩家数据[v.主动] then
							...
							if 主动队伍id ~= 0 then --进入队伍时也要判断
								for n=1,#队伍数据[主动队伍id].成员数据 do
									...
									玩家数据[成员id].道具:给予道具(成员id,"无常勾魂索",1,nil,"不存共享")
								end
							else
								发送数据(玩家数据[v.主动].连接id,3709)
								玩家数据[v.主动].道具:给予道具(v.主动,"无常勾魂索",1,nil,"不存共享")
							end
				    	end
				    	if 玩家数据[v.被动] then
				    		玩家数据[v.被动].勾魂索中=nil
			...
			for k,v in pairs(天罚名单) do
				if 玩家数据[v.ID] and 玩家数据[v.ID].zhandou==0 and 取随机数(1,3)<3 then
