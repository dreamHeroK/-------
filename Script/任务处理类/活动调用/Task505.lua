...
-- @Last Modified time: 2025-11-04 19:58:25
-- @作者: baidwwy
-- @邮箱:  313738139@qq.com
-- @创建时间:   2021-01-31 21:47:44
-- @最后修改来自: baidwwy
-- @Last Modified time: 2023-06-07 00:07:22
local sj = 取随机数
...
function 添加摇钱树元宝任务(id,xy)
  for n=1,取随机数(1,2) do
    ...
    -- local 坐标=地图处理类.地图坐标[玩家数据[id].角色.地图数据.编号].寻路:最近坐标(xy)
...
__GWdh111[6505]=function(连接id,数字id,序列,标识,地图)
	...
	if 数字id==任务数据[标识].玩家id then
		地图处理类:删除单位(任务数据[标识].地图编号,任务数据[标识].单位编号)
		任务数据[标识]=nil
		玩家数据[数字id].角色:添加经验(20000*HDPZ["摇钱树"].经验,"摇钱树",1)
		玩家数据[数字id].角色:添加银子(10000*HDPZ["摇钱树"].银子,"摇钱树",1)
		local 链接 = {提示=format("#S(摇钱树)#G/%s#W轻轻的摇了摇树枝，竟然掉落了",玩家数据[数字id].角色.名称),频道="xt",结尾="#"..取随机数(1,110)}
		local 名称,数量,参数=糖果生成产出(糖果产出物品计算(HDPZ["摇钱树"].ITEM),"摇钱树")
		if 数量== 9999 then --环
			玩家数据[数字id].道具:给予道具(数字id,nil,nil,nil,"不存共享",nil,名称,nil,nil,nil,链接)
		else
