...
__GWdh111[211]=function(连接id,数字id,序列,标识,地图)
	local 对话数据={}
  	对话数据.模型=任务数据[标识].模型
  	对话数据.名称=任务数据[标识].名称
...
__GWdh222[211]=function (连接id,数字id,序号,内容)
  local 事件=内容[1]
  local 名称=内容[3]
  if 事件=="驱年兽过大年" then
    if 任务数据[玩家数据[数字id].地图单位.标识].zhandou~=nil then 常规提示(数字id,"#Y/对方正在战斗中") return  end
    if 取队伍人数(数字id)<1  and 调试模式==false then 常规提示(数字id,"#Y挑战知了王最少必须由五人进行") return  end
    if 取等级要求(数字id,30)==false and 调试模式==false then 常规提示(数字id,"#Y/队伍中有成员等级不符合要求") return  end
    战斗准备类:创建战斗(数字id+0,100097,玩家数据[数字id].地图单位.标识)
...
function 设置任务211()
  ...
  x待发送数据[地图]={}
  for i=1,数量 do
    local 任务id="_"..i.."_"..取随机数(11,88).."_211_"..os.time().."_"..随机序列.."_"..取随机数(88,99999999)
    随机序列=随机序列+1
    local xy=地图处理类.地图坐标[地图]:取随机点()
    任务数据[任务id]={
      id=任务id,
  ...
  x待发送数据={}
  广播消息({内容=format("#W/一群捣乱的年兽出现在了#G/%s#W/处捣乱，还请各位英雄侠士赶紧去降服它们。",取地图名称(地图)),频道="xt"})
end

function rwgx211(任务id)
