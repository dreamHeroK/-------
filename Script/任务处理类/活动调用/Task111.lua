...
-- @Last Modified time: 2025-07-25 16:52:22
-- @作者: baidwwy
-- @邮箱:  313738139@qq.com
-- @创建时间:   2021-01-31 22:11:16
-- @最后修改来自: baidwwy
-- @Last Modified time: 2023-12-10 12:53:27
local sj = 取随机数
...
function 完成任务_111(id,类型,双倍)
	...
	local 奖励次数 = 1
	local 等级=玩家数据[id].角色.等级
	local 基础经验=qz(等级*1000+5000)
	local 基础银子=qz(等级*200)
	local 队伍id=玩家数据[id].队伍
	if 队伍id~=0 then
	    for n=1,#队伍数据[队伍id].成员数据 do
			local cyid = 队伍数据[队伍id].成员数据[n]
			...
			if 取每日限制(cyid,"师门任务") then
				...
				if not 取每日限制(cyid,"师门任务") then --次数是否达到上限
					local 额外银子=qz(等级*等级+100000)
					local 额外经验=等级*等级+20000
				...
				elseif 玩家数据[cyid].角色.师门数据.次数==9 or 玩家数据[cyid].角色.师门数据.次数==10 then
					...
					添加活跃(cyid,10)
						...
						elseif 奖励参数<=50 then
							...
							玩家数据[cyid].道具:给予道具(cyid,"九转金丹",1,取随机数(100,200))
						end
					end
				end
			...
			if 玩家数据[id].角色.师门数据.次数==10 then
				添加活跃(id,10)
					...
					elseif 奖励参数<=50 then
						...
						玩家数据[id].道具:给予道具(id,"九转金丹",1,取随机数(100,200))
					end
				end
			end
