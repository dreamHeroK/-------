...
-- @最后修改来自: baidwwy
-- @Last Modified time: 2026-06-06 22:22:19
local sj = 取随机数
...
function 设置任务8(id)
	...
	if 玩家数据[id].队伍~=nil and 玩家数据[id].队伍~=0 then
		for i=1,#队伍数据[玩家数据[id].队伍].成员数据 do
			...
			设置任务次数限制(队伍数据[玩家数据[id].队伍].成员数据[i],"抓鬼记录")
		end
	end
	任务数据[任务id]={
		领取人id=x临时人物队伍,
...
function 任务说明8(玩家id,任务id) --前往#Y/%s(%s,%s)#W/#L/处降服#G/%s#R(当前第%s次)
	local 说明 = {}
...
function 设置任务14(id)
	...
	for n=1,#队伍数据[玩家数据[id].队伍].成员数据 do
		local 临时id=队伍数据[玩家数据[id].队伍].成员数据[n]
 		...
 		if 判断限制次数(临时id,"自动鬼王") then----限制刷怪
			常规提示(id,"#Y/队伍中"..玩家数据[临时id].角色.名称.."今日鬼王次数已达到30次，无法再领取此任务")
		    return
		end
	end
...
function 设置任务19(id,mcc,mxx)
	...
	-- 发送数据(玩家数据[id].连接id,1501,{名称=mcc,模型=mxx,对话=format("烦请少去"..self.数据[道具id].地图名称.."做场法事,超度那些游荡在三界边缘的孤魂亡灵,我已对你们施展了法术，可逼退他们的怨念，这一路将不会有我的同类来骚扰你们了。")})
end
