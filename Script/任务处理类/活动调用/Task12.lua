...
-- @最后修改来自: baidwwy
-- @Last Modified time: 2025-10-21 23:06:54
local sj = 取随机数
...
function GetUpMOB12(id)
	...
	elseif 取队伍人数(id)<1 then
		...
		return
	-- elseif 取队伍最高等级(玩家数据[id].队伍,120) then
	-- 	常规提示(id,"#Y/等级大于于120级的玩家无法领取此任务")
	-- 	return
	end
	...
	for n=1,#队伍数据[队伍id].成员数据 do
		...
		发送数据(玩家数据[临时id].连接id,1501,{名称="御林军左统领",模型="护卫",对话=format("据可靠消息，偷盗皇宫宝物的 %s 正躲藏 %s(%s,%s) 附近，请立即前去缉拿(已完成%s轮)。",任务数据[任务id].名称,任务数据[任务id].地图名称,任务数据[任务id].x,任务数据[任务id].y,皇宫飞贼.贼王[临时id])})
	end
end
...
function AgainUpMOB12(id)
	if 玩家数据[id].队伍==0 or 玩家数据[id].队长==false  then
		常规提示(id,"#Y/该任务必须组队完成且由队长领取")
		return
	elseif 取队伍最低等级(玩家数据[id].队伍,80) then
		常规提示(id,"#Y/等级小于80级的玩家无法领取此任务")
		return
	elseif 取队伍人数(id)<1 then
		常规提示(id,"#Y/本任务至少需要五人组队完成")
		return
	...
	for n=1,#队伍数据[队伍id].成员数据 do
		...
		if 皇宫飞贼.贼王[临时id]==nil or 皇宫飞贼.贼王[临时id]<5 then
			添加最后对话(id,format("缉拿贼王需要先完成5轮飞贼任务,#G%s#W尚未满足条件。已完成%s轮",玩家数据[临时id].角色.名称,皇宫飞贼.贼王[临时id]))
			return
