...
function 文韵墨香:开启活动()
		...
		-- self.开启Time=os.time()+4200 --1.1小时
	-- else
...
function 文韵墨香:添加文墨任务(id)
	...
	elseif 任务分类==6 then --各门派送信
		...
		玩家数据[id].道具:给予道具(id,"密信",nil,nil,"不存共享","专用")
	...
	for n=1,#队伍数据[队伍id].成员数据 do
		...
		Q_文韵墨香[cyid]=(Q_文韵墨香[cyid] or 0)+1
		添加最后对话(cyid,duihua)
...
function 文韵墨香:任务说明(玩家id,任务id)
	...
	elseif 任务数据[任务id].分类 == 3 then --护送西域使者
		...
		elseif 任务数据[任务id].子类==2 then
			说明={"文韵墨香",format("将#R西域使者#W带到长安城内，再和其对话，当前第%s环。剩余时间"..取分(任务数据[任务id].结束-(os.time()-任务数据[任务id].起始)).."分钟。",次数)}
		end
	elseif 任务数据[任务id].分类 == 4 then --陈员外
		...
		elseif 任务数据[任务id].子类==2 then
			说明={"文韵墨香",format("到".."#R/qqq|王武*镇远武馆*npc查询/王武".."#W处取点木头来。当前第%s环。剩余时间"..取分(任务数据[任务id].结束-(os.time()-任务数据[任务id].起始)).."分钟。",次数)}
		end
...
function 文韵墨香:对话事件处理(id,名称,事件)
	if 事件=="正闲着呢，请吩咐！" then
		...
		elseif 玩家数据[id].队长==false then
			...
			return
		elseif 玩家数据[id].角色:取任务(1163) ~= 0 then
			添加最后对话(id,"少侠身上已有任务，请完成后再来领取！")
			return
		end
		local go=true
		local 队伍id=玩家数据[id].队伍
		for n=1,#队伍数据[队伍id].成员数据 do
			...
			if Q_文韵墨香[cyid] and Q_文韵墨香[cyid]>=30 then  ---5轮30次  明白
...
function 文韵墨香:设置奖励(id)
	if 取随机数()<=HDPZ["文韵墨香"].爆率 then
		...
		local 名称,数量,参数=糖果生成产出(糖果产出物品计算(HDPZ["文韵墨香"].ITEM),"文韵墨香")
		if 数量== 9999 then --环
