...
function 天降星辰:关闭活动(id)
	self.数据={}
	self.活动开关 = false
	self.开启Time = nil
	for i in pairs(任务数据) do
	    if 任务数据[i].类型 == 1165 then
...
function 天降星辰:对话事件处理(id,名称,事件)
	if 事件=="我欲前往收服十二元辰" then
		...
		for n=1,#队伍数据[队伍id].成员数据 do
			...
			-- if Q_天降辰星[cyid] and Q_天降辰星[cyid]>=3 then
			-- 	return 添加最后对话(id,玩家数据[cyid].角色.名称.."今日已经完成了全部任务！")
			-- end
		end
		if go then
			self:新增任务(id)
...
__GWdh222[1165]=function (连接id,id,序号,内容)
	...
	if 事件=="待我一战！" then
		...
		if not 天降辰星.活动开关 then
			玩家数据[id].角色:取消任务(玩家数据[id].角色:取任务(1166))
			添加最后对话(id,"活动已结束，请下次再来参与")
			return
		end
		local 任务id=玩家数据[id].角色:取任务(1166)
		if 任务id~=0 then
...
-- 				发送数据(玩家数据[cyid].连接id,1501,{名称="游奕灵官",模型="天兵",对话=format("果然身手不错！此番十二元辰挑战你已经全部完成，回去找到游奕灵官（长安城，128，97）继续新的挑战吧！#81")})
-- 			else
...
function 天降星辰:战斗胜利处理(id组,战斗类型)
	...
	for n=1,#id组 do
		local cyid=id组[n]
		-- 成就数据[cyid]:判断进度(cyid,"日常任务","星辰300次")
		-- 成就数据[cyid]:判断进度(cyid,"限时活动","星辰100次")
		if self.活动开关 and self.数据[cyid] then
			self.数据[cyid].次数=self.数据[id].次数
			if self.数据[cyid].次数>12 then
				local 链接 = {提示=format("#P(天降辰星)#G/%s#W完成了又一轮十二元辰挑战，特赠送",玩家数据[cyid].角色.名称),频道="hd",结尾="#W以资鼓励！"}
				local 名称="九转金丹"
				玩家数据[cyid].道具:给予超链接道具(cyid,名称,1,100,链接,"成功")
				self.数据[cyid].环数=self.数据[cyid].环数+1
				玩家数据[cyid].角色:取消任务(玩家数据[cyid].角色:取任务(1166))
				发送数据(玩家数据[cyid].连接id,1501,{名称="游奕灵官",模型="天兵",对话=format("果然身手不错！此番十二元辰挑战你已经全部完成，回去找到游奕灵官（长安城，128，97）继续新的挑战吧！#81")})
			else
			...
			elseif 等级 <= 89   then jlxs = 1
			elseif 等级 <= 109 then jlxs = 2
			elseif 等级 <= 129 then jlxs = 2
			elseif 等级 <= 155 then jlxs = 3
			elseif 等级 <= 175 then jlxs = 3
			end
			local 经验=等级*jlxs*取随机数(1500,2500)
			local 银子=等级*jlxs*170+10000
			玩家数据[cyid].角色:添加经验(经验*HDPZ["天降辰星"].经验,"天降辰星",1)
			玩家数据[cyid].角色:添加银子(银子*HDPZ["天降辰星"].银子,"天降辰星",1)
			玩家数据[cyid].角色:添加储备(银子*3*HDPZ["天降辰星"].银子,"天降辰星",1)
