...
-- @Last Modified time: 2024-01-04 13:54:01
local 押镖 = class()
...
function 押镖:完成押镖任务(任务id,id,地图)
	...
	玩家数据[id].角色:添加银子(qz(押镖银子),"押镖",1)
	-- 玩家数据[id].角色:添加活跃度(id,1)
	if self.数据[id] and self.数据[id].次数< 50 then
	    if 取随机数()<=15 then
	    	local 链接 = {提示=format("#S(押镖任务)#G/%s#Y顺利完成押送镖银任务，郑镖头奖励了他一本",玩家数据[id].角色.名称),频道="xt",结尾="#"..取随机数(1,110).."#Y！"}
	    	玩家数据[id].道具:给予超链接道具(id,"鬼谷子",nil,nil,链接)
	    end
	else
		常规提示(id,"#Y/今日押镖已达50次，不会再获得物品奖励！")
	end
