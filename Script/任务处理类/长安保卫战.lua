...
-- @Last Modified time: 2026-05-30 10:10:10
local 长安保卫战 = class()
...
function 长安保卫战:刷出怪物()
	...
	if 取随机数()<=50 then
		...
		self.刷怪记录[#self.刷怪记录+1]="#M"..时间转换(os.time()).."#K"..dtmc.."#M出现了一批前来捣乱的妖魔，各路豪杰请前去消灭他们，前往不要放走过一个。\n"
	...
	if 战斗类型==130087 then  --先锋
		for n=1,#id组 do
			...
			if 取随机数()<=HDPZ["保卫长安：小怪先锋"].爆率 then
				...
				local 名称,数量,参数=糖果生成产出(糖果产出物品计算(HDPZ["保卫长安：小怪先锋"].ITEM),"保卫长安：小怪先锋")
				if 数量== 9999 then --环
	...
	elseif 战斗类型==130088 then  --头目
		for n=1,#id组 do
			...
			if 取随机数()<=HDPZ["保卫长安：小怪头目"].爆率 then
				 local 链接 = {提示=format("#S(长安保卫战)#Y意外惊喜，/%s#Y在抵御西方妖魔的战斗中额外获得了",玩家数据[id].角色.名称),频道="xt",结尾="#Y一个。#80"}
				local 名称,数量,参数=糖果生成产出(糖果产出物品计算(HDPZ["保卫长安：小怪头目"].ITEM),"保卫长安：小怪头目")
				if 数量== 9999 then --环
					玩家数据[id].道具:给予道具(id,nil,nil,nil,nil,nil,名称,nil,nil,nil,链接)
				else
					玩家数据[id].道具:给予超链接道具(id,名称,数量,参数,链接)
	...
	elseif 战斗类型==130089 then  --大王
		for n=1,#id组 do
			...
			if 取随机数()<=HDPZ["保卫长安：小怪大王"].爆率 then
				 local 链接 = {提示=format("#S(长安保卫战)#Y意外惊喜，/%s#Y在抵御西方妖魔的战斗中额外获得了",玩家数据[id].角色.名称),频道="xt",结尾="#Y一个。#80"}
				local 名称,数量,参数=糖果生成产出(糖果产出物品计算(HDPZ["保卫长安：小怪大王"].ITEM),"保卫长安：小怪大王")
				if 数量== 9999 then --环
					玩家数据[id].道具:给予道具(id,nil,nil,nil,nil,nil,名称,nil,nil,nil,链接)
				else
					玩家数据[id].道具:给予超链接道具(id,名称,数量,参数,链接)
	...
	elseif 战斗类型>=130090 and 战斗类型<=130093 then  --保卫长安_魑    保卫长安_魅    保卫长安_魍    保卫长安_魉
		for n=1,#id组 do
			...
			if 取随机数()<=HDPZ["保卫长安：魑魅魍魉"].爆率 then
				...
				local 名称,数量,参数=糖果生成产出(糖果产出物品计算(HDPZ["保卫长安：魑魅魍魉"].ITEM),"保卫长安：魑魅魍魉")
				if 数量== 9999 then --环
					玩家数据[id].道具:给予道具(id,nil,nil,nil,nil,nil,名称,nil,nil,nil,链接)
				else
					玩家数据[id].道具:给予超链接道具(id,名称,数量,参数,链接)
	...
	elseif 战斗类型>=130094 and 战斗类型<=130095 then  --保卫长安_鼍龙    保卫长安_兕怪
		for n=1,#id组 do
			...
			if 取随机数()<=HDPZ["保卫长安：鼍龙兕怪"].爆率 then
				local 链接 = {提示=format("#S(长安保卫战)#G/%s#Y勇闯魔窟，打的西方妖魔落荒而逃，并缴获一份",玩家数据[id].角色.名称),频道="xt",结尾="#Y！#24"}
				local 名称,数量,参数=糖果生成产出(糖果产出物品计算(HDPZ["保卫长安：鼍龙兕怪"].ITEM),"保卫长安：鼍龙兕怪")
				if 数量== 9999 then --环
					玩家数据[id].道具:给予道具(id,nil,nil,nil,nil,nil,名称,nil,nil,nil,链接)
				else
					玩家数据[id].道具:给予超链接道具(id,名称,数量,参数,链接)
