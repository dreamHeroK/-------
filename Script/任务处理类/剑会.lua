...
-- @Last Modified time: 2025-08-11 15:38:14
local 剑会 = class()
...
function 剑会:开启活动()
	self.模式="单挑"
	self.正式开始=false
	self.开始Time=os.time()+5400 --前十分钟是准备时间
	if 调试模式 then
	self.正式Time=os.time()+10 --开启后10分钟后正式开启PK
	else
	self.正式Time=os.time()+600 --开启后10分钟后正式开启PK
	end
	self.活动开关=true
	self.单挑={}
	广播消息({内容="#R《剑会天下·单挑赛》#G活动开启，三界之中，谁是英豪！凡三界之侠士，俱可前往其中一试身手。#Y（长安欧冶子之灵142，177处）",频道="xt"})
	发送公告("#R《剑会天下·单挑赛》#G活动开启，三界之中，谁是英豪！凡三界之侠士，俱可前往其中一试身手。")
end
...
function 剑会:PK胜利处理(胜利组,失败组)
	if 胜利组 and 失败组 and self.jifenban[胜利组[1]] and self.jifenban[失败组[1]] then
		...
		if 取随机数()<=HDPZ["剑会：失败"].爆率 then
			...
			local 名称,数量,参数=糖果生成产出(糖果产出物品计算(HDPZ["剑会：失败"].ITEM),"剑会：失败")
			if 数量== 9999 then --环
...
function 剑会:对话事件处理(id,名称,事件)
	if 名称=="欧冶子之灵" or 名称=="欧冶子" then
		if 事件=="我要参加剑会天下" then
			-- 添加最后对话(id,"请问，少侠要前往参加哪种剑会天下游戏模式：",{"剑会天下（普通模式）","剑会天下（单挑模式）"})
			添加最后对话(id,"请问，少侠要前往参加哪种剑会天下游戏模式：",{"剑会天下（单挑模式）"})
		...
		elseif 事件=="剑会天下（普通模式）" then
			添加最后对话(id,"这个模式还在开发中……")
		elseif 事件=="了解剑会天下" then
			添加最后对话(id,"少侠要前了解哪种剑会天下游戏模式：",{"普通模式","单挑模式"})
		elseif 事件=="普通模式" then
			添加最后对话(id,"这个模式还在开发中……")
		elseif 事件=="单挑模式" then
			添加最后对话(id,"于太上老君处习得#Y“单挑诀”#W的玩家可与各门派精英展开一对一的对决，对决中可#Y召唤最多4个#W召唤兽同时在场。\n#Y单人即可免费参与，活动中不消耗药品，装备耐久，宝宝寿命等。")
		elseif 事件=="查看排行榜" then
			self:剑会排行榜(id)
		elseif 事件=="兑换奖励" then
			添加最后对话(id,"少侠没有排行榜奖励或者单挑礼包奖励可以领取。")
...
function 剑会:计算积分称谓(id)
	local qwe
	if self.jifenban[id].积分>=1200 and self.jifenban[id].积分<1300 then
		qwe="剑会天下·新秀"
