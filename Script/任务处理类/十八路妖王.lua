...
local 刷新数据 ={
	[1]={名称="黄风怪",模型="妖王九尾狐狸"},[2]={名称="白骨精",模型="妖王九尾狐狸"},[3]={名称="狮猁怪",模型="妖王九尾狐狸"},[4]={名称="眼魔君",模型="妖王九尾狐狸"},
	[5]={名称="蝎子精",模型="妖王九尾狐狸"},[6]={名称="红孩儿",模型="妖王九尾狐狸"},[7]={名称="六耳猕猴",模型="妖王九尾狐狸"},[8]={名称="玉兔精",模型="妖王九尾狐狸"},
	[9]={名称="独角兕大王",模型="妖王九尾狐狸"},[10]={名称="混世魔王",模型="妖王九尾狐狸"},[11]={名称="黑风怪",模型="妖王九尾狐狸"},[12]={名称="九尾狐狸",模型="妖王九尾狐狸"},
	[13]={名称="妖王之主",模型="蚩尤"},
}
...
function 十八妖王:活动定时器()
  if 服务端参数.分钟+0==0 and os.time() - self.检测已刷 >= 0 then
     ...
     self:十八路妖王()
  elseif os.time() - self.检测已刷 >= 120 then
     -- 活动结束，移除所有未击败的怪物
     for id, 任务 in pairs(任务数据) do
       if 任务.类型 == 1255 then
         地图处理类:删除单位(id)
         任务数据[id] = nil
       end
...
if 任务数据[任务id].名称 == "黄风怪" then
    ...
    -- 战斗结束后移除怪物
    if 任务数据[任务id].地图编号 and 地图处理类.地图单位[任务数据[任务id].地图编号] then
      地图处理类:删除单位(任务id)
    end

    -- 更新妖王排行
    local 类型 = 任务数据[任务id].名称
    if 十八路妖王[类型] == nil then
      十八路妖王[类型] = {}
    end
    ...
    -- for i=1,#id组 do
    --   local id = id组[i]
    --   if 战斗数据[战斗类型].伤害统计[id] and 战斗数据[战斗类型].伤害统计[id] > 最高伤害 then
    --     最高伤害 = 战斗数据[战斗类型].伤害统计[id]
    --     最高伤害玩家 = {
    --       ID = id,
    --       名称 = 玩家数据[id].角色.名称,
    --       等级 = 玩家数据[id].角色.等级,
