...
-- @Last Modified time: 2025-11-21 14:54:27
local 门派闯关 = class()
...
__GWdh222[106]=function (连接id,id,序号,内容)
	...
	if 事件=="请出招吧" then
		...
		if not 调试模式 and ( 玩家数据[id].队伍==0 or 取队伍人数(id)<3 or 取队伍最低等级(玩家数据[id].队伍,40)) then --测试模式1210
		...
		if 玩家数据[id].角色.地图数据.编号~=任务数据[玩家数据[id].地图单位.标识].地图编号 or 取两点距离(比较xy,假人数据)>20 then
			...
			print("门派闯关:脚本距离="..取两点距离(比较xy,假人数据))
			return
		end
...
function 门派闯关:战斗胜利处理(id组,战斗类型)
	...
	for n=1,#id组 do
		...
		if qx then
			...
			-- 	玩家数据[cyid].道具:给予超链接书铁(cyid,{13,13},nil,链接1,"成功")
			-- end
		else
			self:设置奖励(cyid)
			玩家数据[cyid].角色:刷新任务跟踪()
...
function 门派闯关:设置奖励(id)
	if 取随机数()<=HDPZ["门派闯关"].爆率 then
		local 链接 = {提示=format("#P(门派闯关)#R%s#Y在门派闯关活动中喜获",玩家数据[id].角色.名称),频道="hd",结尾="#Y一个。"}
						local 名称,数量,参数=糖果生成产出(糖果产出物品计算(HDPZ["门派闯关"].ITEM),"门派闯关")
						if 数量== 9999 then --环
