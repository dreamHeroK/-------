...
-- @Last Modified time: 2025-11-21 14:49:08
local 彩虹争霸 = class()
...
function 彩虹争霸:开启报名()
	...
	self.正式Time=os.time()+10 --15后分钟
	-- if 调试模式 then
	--     self.进场Time=os.time()+2 --10后分钟
	-- 	self.正式Time=os.time()+3 --15后分钟
	-- end
	self.活动开关=true
	self.进场开关 = false
	self.闯关开关 = false
	...
	self.云影玩家 = 0
	self.虹光玩家 = 0
	发送公告("#G(彩虹争霸)#P已经开启，请提前20分钟到朱紫国的彩虹大使处报名参赛！")
...
function 彩虹争霸:计算胜利方()
	if self.阵营分数.云影>=self.阵营分数.虹光 then
		for k,v in pairs(self.云影成员) do
			if self.数据[k]~=nil and self.数据[k].阵营 == "云影" then
				self.数据[k].胜利=true
			end
		end
		self.胜利方 = "云影阵营"
		...
		for k,v in pairs(self.虹光成员) do
			if self.数据[k]~=nil and self.数据[k].阵营 == "虹光" then
				self.数据[k].胜利=true
			end
		end
		self.胜利方 = "虹光阵营"
...
function 彩虹争霸:战斗胜利处理(任务id,id组,战斗类型)
	...
	if 战斗类型>=130023 and 战斗类型<=130029 then  --传送大使
		...
		elseif 等级 <= 89   then jlxs = 1.5
		elseif 等级 <= 109 then jlxs = 2
		elseif 等级 <= 129 then jlxs = 3
		elseif 等级 <= 155 then jlxs = 4
		elseif 等级 <= 175 then jlxs = 5
		end
			local 经验=等级*jlxs*取随机数(800,900)+30000
			local 银子=等级*jlxs*200+15000--37250
			...
			local 名称,数量,参数=糖果生成产出(糖果产出物品计算(HDPZ["彩虹：传送大使"].ITEM),"彩虹：传送大使")
			if 数量== 9999 then --环
	...
	elseif 战斗类型>=130002 and 战斗类型<=130015 then  --各个场景怪
		...
		elseif 等级 <= 175 then jlxs = 5
			...
			local 链接 = {提示=format("#S(彩虹争霸)#G/%s#Y舍己为人，战斗在最前线，为大家扫清通关的道路，奖励".."#"..取随机数(1,110),玩家数据[id].角色.名称),频道="hd",结尾="#Y一个！"}
							local 名称,数量,参数=糖果生成产出(糖果产出物品计算(HDPZ["彩虹：场景小怪"].ITEM),"彩虹：场景小怪")
							if 数量== 9999 then --环
								玩家数据[id].道具:给予道具(id,nil,nil,nil,nil,nil,名称,nil,nil,nil,链接)
							else
										玩家数据[id].道具:给予超链接道具(id,名称,数量,参数,链接)
	...
	elseif 战斗类型>=130016 and 战斗类型<=130022 then  --顶层泡泡
		...
		elseif 等级 <= 175 then jlxs = 5
			...
			local 经验=等级*jlxs*取随机数(800,900)+100000
			local 银子=等级*jlxs*200+30000--93300
			玩家数据[id].角色:添加经验(经验*HDPZ["彩虹：仙境泡泡"].经验,"彩虹闯关",1)
			...
			if 取随机数()<=HDPZ["彩虹：仙境泡泡"].爆率 then
				local 链接 = {提示=format("#S(彩虹争霸)#G/%s#Y成功的战胜了顶层的可爱泡泡，获得一个",玩家数据[id].角色.名称),频道="hd",结尾="#Y奖励，不愧为登顶的勇士啊！".."#"..取随机数(1,110)}
								local 名称,数量,参数=糖果生成产出(糖果产出物品计算(HDPZ["彩虹：仙境泡泡"].ITEM),"彩虹：仙境泡泡")
								if 数量== 9999 then --环
									玩家数据[id].道具:给予道具(id,nil,nil,nil,nil,nil,名称,nil,nil,nil,链接)
								else
											玩家数据[id].道具:给予超链接道具(id,名称,数量,参数,链接)
...
-- function 彩虹争霸:战斗胜利处理(任务id,id组,战斗类型)
-- 	if 战斗类型>=130023 and 战斗类型<=130029 then  --传送大使
-- 		self.数据[id组[1]].层数=(self.数据[id组[1]].层数 or 0)+1
-- 		for n=1,#id组 do
-- 			local id=id组[n]
-- 			玩家数据[id].zhandou=0
-- 			if self.数据[id组[n]] then
...
-- 	常规提示(id,"#Y这个宝箱已经被人抢走了！")
-- 	return
--   elseif self.宝箱开关 == false then
--   	常规提示(id,"#Y活动已经结束，无法继续开启宝箱了！")
--   	return
--   end
--   if self.数据[id]~=nil and self.数据[id].阵营 ~= nil and self.数据[id].胜利 then
-- 	地图处理类:删除单位(任务数据[任务id].地图编号,任务数据[任务id].单位编号)
