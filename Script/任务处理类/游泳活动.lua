...
-- @Last Modified time: 2025-11-21 14:53:15
local 游泳活动 = class()
...
function 游泳活动:完成游泳任务(id)
	...
	for n=1,#队伍数据[队伍id].成员数据 do
		...
		local 经验=qz(等级*取随机数(2000,2200))
		local 银子=qz(等级*取随机数(2000,2100))
		local 储备=等级*取随机数(2500,3000)+30000
		玩家数据[cyid].角色:添加储备(储备,"游泳比赛",1)
		玩家数据[cyid].角色:添加经验(经验*HDPZ["游泳比赛"].经验,"游泳比赛",1)
		玩家数据[cyid].角色:添加银子(qz((银子+10000)*0.6*HDPZ["游泳比赛"].银子),"游泳比赛",1)
		...
		if 取随机数()<=HDPZ["游泳比赛"].爆率 then
			local 链接={提示=format("#S(游泳比赛)#G/%s#Y经过一番激烈的追逐和竞争，游泳试练官员随手奖励了他",玩家数据[cyid].角色.名称),频道="hd",结尾="#Y一个！".."#"..取随机数(1,110)}
							local 名称,数量,参数=糖果生成产出(糖果产出物品计算(HDPZ["游泳比赛"].ITEM),"游泳比赛")
							if 数量== 9999 then --环
