...
-- @Last Modified time  : 2024-01-14 17:21:19

-- @Author: baidwwy
-- @Date:   2023-09-23 09:20:04
-- @Last Modified by:   baidwwy
-- @Last Modified time: 2026-02-19 08:38:15
...
function 帮派处理:手动升级建筑()
    	for k,v in pairs(取所有帮派) do
		local 建筑类型列表 = {"金库", "书院", "兽室", "厢房", "药房", "仓库"}
                	for _, 建筑类型 in ipairs(建筑类型列表) do
	                    local 建筑数据 = 帮派数据[k].帮派建筑[建筑类型]
	                    local 升级前等级 = 建筑数据.数量 -- 记录初始等级
	                    local 升级后等级 = 升级前等级 -- 初始化最终等级
	                    while true do
	                        local 所需升级经验 = self.帮派建筑升级经验[升级后等级]
	                        if not 所需升级经验 or 建筑数据.当前经验 < 所需升级经验 then
	                            break
	                        end
	                        升级后等级 = 升级后等级 + 1
	                        建筑数据.当前经验 = 建筑数据.当前经验 - 所需升级经验
	                    end
	                    if 升级后等级 > 升级前等级 then
	                        建筑数据.数量 = 升级后等级
	                        广播帮派消息({
	                            内容 = "[帮派维护]#G/帮派"..建筑类型.."从"..升级前等级.."个自动升级至"..升级后等级.."个！",
	                            频道 = "bp"
	                        }, 帮派数据[k].帮派编号)
	                    end
	                end
    ...
    for k,v in pairs(取所有帮派) do
		if not v.已解散 then
			if 帮派数据[k].帮派资金.当前 <= 10000000 then--帮派数据[k].帮派资金.上限*0.2 then
				...
				帮派数据[k].帮派资材.当前 = 帮派数据[k].帮派资材.当前 + 200000
				...
				-- if 帮派数据[k].研究技能 ~= nil then
				-- 	local 加成 = 100
				-- 	帮派数据[k].技能数据[帮派数据[k].研究技能].当前经验 = 帮派数据[k].技能数据[帮派数据[k].研究技能].当前经验 + 加成 + 帮派数据[k].帮派建筑.书院.数量*1000
				-- end
				-- if 帮派数据[k].研究修炼 ~= nil then
				-- 	local 加成 = 50
				-- 	帮派数据[k].修炼数据[帮派数据[k].研究修炼].当前经验 = 帮派数据[k].修炼数据[帮派数据[k].研究修炼].当前经验 + 加成
				-- end
				------------------------------------------------C改
				local 建筑类型列表 = {"金库", "书院", "兽室", "厢房", "药房", "仓库"}
		                	for _, 建筑类型 in ipairs(建筑类型列表) do
...
function 帮派处理:创建帮派(数字id,内容)
	local 帮派创建消耗 = 10000000
	if 老八 then 帮派创建消耗 = 500000000 end
	if 玩家数据[数字id].角色.等级<40 then
		常规提示(数字id,"小于40级不能创建帮派")
		return
	end
	local bpsl=0
	for k,v in pairs(取所有帮派) do
	...
	if 玩家数据[数字id].角色:扣除银子(帮派创建消耗,0,0,"创建帮派",1)== false then 常规提示(数字id,"创建帮派需要"..帮派创建消耗.."两银子！") return end
	local 成员上限 = 500
	if 神话 then 成员上限 = 500 end
	...
	帮派数据[XZBPBH] ={
		...
		技能数据 = {打造技巧={当前=160,上限=160,当前经验=0},
					...
					灵石技巧={当前=120,上限=120,当前经验=0},
					烹饪技巧={当前=160,上限=160,当前经验=0},
					中药医理={当前=160,上限=160,当前经验=0},
					冥想={当前=160,上限=160,当前经验=0},
					强身术={当前=140,上限=140,当前经验=0},
					强壮={当前=80,上限=80,当前经验=0}, --可学习等级=人物等级/2(即160级可学满)
					神速={当前=80,上限=80,当前经验=0} --可学习等级=人物等级/2(即160级可学满)
					},
	...
	帮派数据[XZBPBH].成员数据[数字id] = {
					...
					权限=5,
					id = 数字id,
					帮贡={当前=玩家数据[数字id].角色.BG,上限=20000}, --测试模式
					青龙=0,
					跑商=0,
					迷宫=0,
					厢房=0,
...
function 帮派处理:帮派规模提升(id)
	...
	elseif self:取帮派建筑达标情况(帮派数据[bh].帮派规模,bh) == 2 then
		...
		elseif 帮派数据[bh].帮派规模 == 3 then
			...
			帮派数据[bh].成员数量.上限 = 500
			帮派数据[bh].技能数据.强身术={当前=100,上限=100,当前经验=0}
		...
		elseif 帮派数据[bh].帮派规模 == 4 then
			...
			帮派数据[bh].成员数量.上限 = 500
			帮派数据[bh].技能数据.强身术={当前=110,上限=110,当前经验=0}
		...
		elseif 帮派数据[bh].帮派规模 == 5 then
			...
			帮派数据[bh].成员数量.上限 = 500
			帮派数据[bh].技能数据.强身术={当前=120,上限=120,当前经验=0}
		...
		elseif 帮派数据[bh].帮派规模 == 6 then
			...
			帮派数据[bh].当前维护费 =500000
			帮派数据[bh].成员数量.上限 = 500
			帮派数据[bh].技能数据.强身术={当前=130,上限=130,当前经验=0}
		...
		if 神话 then 帮派数据[bh].成员数量.上限 = 500 end
...
function 帮派处理:增加当前内政(bh,num,提示)
	...
	if 帮派数据[bh] then
		...
		if 帮派数据[bh].帮派建筑[当前内政].数量 < self:取帮派建筑数量(帮派数据[bh].帮派规模) then
			...
			if 帮派数据[bh].帮派建筑[当前内政].当前经验 >= self.帮派建筑升级经验[帮派数据[bh].帮派建筑[当前内政].数量] * 当前升级 then
				帮派数据[bh].帮派建筑[当前内政].当前经验 = 帮派数据[bh].帮派建筑[当前内政].当前经验-self.帮派建筑升级经验[帮派数据[bh].帮派建筑[当前内政].数量] * 当前升级
				...
				elseif 当前内政 == "仓库" then
					if 帮派数据[bh].帮派规模 == 1 then
						帮派数据[bh].帮派资材.上限 =200000--1000+帮派数据[bh].帮派建筑.仓库.数量*100
					elseif 帮派数据[bh].帮派规模 == 2 then
						帮派数据[bh].帮派资材.上限 =200000--1200+帮派数据[bh].帮派建筑.仓库.数量*100
					elseif 帮派数据[bh].帮派规模 == 3 then
						帮派数据[bh].帮派资材.上限 =200000--1400+帮派数据[bh].帮派建筑.仓库.数量*100
					elseif 帮派数据[bh].帮派规模 == 4 then
						帮派数据[bh].帮派资材.上限 =200000--1600+帮派数据[bh].帮派建筑.仓库.数量*100
					elseif 帮派数据[bh].帮派规模 == 5 then
						帮派数据[bh].帮派资材.上限 =200000--1800+帮派数据[bh].帮派建筑.仓库.数量*100
					end
				elseif 当前内政 == "药房" then
					if 帮派数据[bh].帮派规模 == 1 then
...
function 帮派处理:取帮派建筑达标情况(等级,bh)
	...
	elseif 等级 == 6 and 帮派数据[bh].帮派资金.当前 > 13360000 then
		银两符合 = true
	elseif 等级 == 7 and 帮派数据[bh].帮派资金.当前 > 15000000 then
		银两符合 = true
	end
...
function 帮派处理:玩家数据变更(内容) --逐出帮派 任命职位  同意加入
	...
	elseif 操作类型=="任命职位" then
		...
		-- 发送数据(玩家数据[id].连接id,208,{数据=帮派数据[bh],类型=操作类型})
...
function 帮派处理:清空申请(数字id)
	local bh=玩家数据[数字id].角色.BPBH
	if 帮派数据[bh] == nil or 帮派数据[bh].成员数据[数字id] == nil or 帮派数据[bh].成员数据[数字id].权限 < 3 then
		常规提示(数字id,"你没有权限执行此操作")
		return
	end
	帮派数据[bh].申请人员 = {}
