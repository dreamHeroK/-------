...
-- @Last Modified time  : 2024-01-14 15:24:37

-- @Author: baidwwy
-- @Date:   2023-09-12 12:17:35
-- @Last Modified by:   baidwwy
-- @Last Modified time: 2024-01-12 21:31:09
-- @Author: baidwwy
-- @Date:   2023-09-12 12:17:35
-- @Last Modified by:   baidwwy
-- @Last Modified time: 2023-09-19 16:40:39
...
function 神器类:数据处理(连接id,序号,id,内容) --序号>6200 and 序号<=6300
	...
	elseif 序号==6202.1 then
		if 玩家数据[id].角色:扣除银子(1000000,0,0,"炼制灵犀之屑",1) then
		    玩家数据[id].道具:给予道具(id,"灵犀之屑",1)
		else
			添加最后对话(id,"解锁这个部件需消耗1000000两梦幻币。")
		end
	...
	elseif 序号==6204 then --解锁其他部件
		if 玩家数据[id].角色:扣除银子(5000000,0,0,"神器解锁部件",1) then
		    local 区域=内容.区域
	...
	elseif 序号==6215 then --确定紅 --神器激活插槽
		if self.数据.神器解锁点 == nil then
			self.数据.神器解锁点 = 0
		end
		if tonumber(self.数据.神器解锁点) < 100 then
    ...
    elseif 序号==6216 then
		...
		if 玩家数据[id].角色:扣除银子(yz,0,0,"神器解锁点",1) then
...
function 神器类:切换神器(id)
	...
	if not  玩家数据[id].神器.数据.是否有 then
		添加最后对话(id,"你连神器都没有！切！#24")
		return
	end
  if 玩家数据[id].角色:扣除银子(5000000,0,0,"切换神器",1) then
