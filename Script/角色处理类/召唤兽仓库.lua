...
-- @Date                : 2024-01-10 19:38:59
-- @Last Modified time  : 2024-01-14 15:25:11

-- @Author: baidwwy
-- @Date:   2023-11-28 22:56:30
-- @Last Modified by:   baidwwy
-- @Last Modified time: 2024-01-10 13:46:32
local 召唤兽仓库类 = class()
...
function 召唤兽仓库类:加载数据(账号,数字id)
    if f函数.文件是否存在([[data/]]..账号..[[/宝宝仓库.txt]])==false then
    self.数据={[1]={},[2]={},[3]={}}
    写出文件([[data/]]..账号.."/宝宝仓库.txt",table.tostring(self.数据))
    else
    self.数据=table.loadstring(读入文件([[data/]]..账号..[[/宝宝仓库.txt]]))
    end
end
...
function 召唤兽仓库类:仓库存取(连接id,数字id,内容)
	...
	if 类型 == "存" then
		...
		if 是否存在 and bianh~=nil then
			...
			玩家数据[数字id].角色:日志记录(format("[召唤兽系统-仓库存取]存入角色ID[%s]召唤兽认证码[%s]",数字id,bb.认证码), "召唤兽仓库存")
	...
	elseif 类型 =="取" then
		if 玩家数据[数字id].角色:取新增宝宝数量() then
			...
			if 是否存在 and bianh~=nil then
				...
				玩家数据[数字id].角色:日志记录(format("[召唤兽系统-仓库存取]取出角色ID[%s]召唤兽认证码[%s]",数字id,self.数据[页数][bianh].认证码) , "召唤兽仓库取")
