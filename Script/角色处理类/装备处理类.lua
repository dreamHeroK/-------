...
function 装备处理类:生成打造装备假人(id,级别,种类,名称,生产方式,制造者,是否鉴定,专用,无级别,对方id,返回) --真正的强化打造公式
	...
	local dz = self:装备万能公式(级别,种类,生产方式,专用,对方id,nil,id)
	local 道具 = 物品类()
...
function 装备处理类:生成打造装备(id,级别,种类,名称,生产方式,制造者,是否鉴定,专用,无级别,对方id,返回) --真正的强化打造公式
	local dz = self:装备万能公式(级别,种类,生产方式,专用,对方id,返回,id)
	if 级别 == 160 then dz = self:装备万能公式(级别,种类,生产方式,专用,对方id, 返回 or 1.1,id) end
	local 道具 = 物品类()
	道具:置对象(名称,nil,nil,级别)
	道具.等级 = 级别
	if 种类 ~= nil then
...
function 装备处理类:发送定制装备(id,级别,种类,名称,生产方式,制造者,是否鉴定,专用,无级别,对方id,倍率,特技,双加,套装类型,套装效果) --真正的强化打造公式
	local dz = self:装备万能公式(级别,种类,生产方式,专用,对方id,倍率,id)
	local 道具 = 物品类()
	道具:置对象(名称,nil,nil,级别)
	道具.等级 = 级别
	if 种类 ~= nil then
...
function 装备处理类:装备万能公式(a,b,生产方式,专用,对方id,属性倍率,id) -- 制作书等级 制作种类  --真正的强化打造公式
	local 附加属性 = false
	...
	local 通用特效 = {}
	local 倍率=属性倍率 or 1
	local 白名单满值 = id and 玩家数据[id] and 满值白名单账号[玩家数据[id].账号] --ponytail: 白名单账号打造装备取最大值，别的账号别碰
	local shangxian = 白名单满值 and 1 or 取随机数(90,100)*0.01
	local function qsz(min,max) -- 满值白名单直接取上限，否则按原随机公式
		if 白名单满值 then return max end
		return 取随机数(min,max)
	end
	if b < 19 then -- 武器
		local 命中,伤害 =self.装备数据.武器.基础属性(a,生产方式)
		c[1] = qz(qsz(命中[1],命中[2]*shangxian)*倍率)--qz(命中[2]*0.8)
		c[2] = qz(qsz(伤害[1],伤害[2]*shangxian)*倍率)--qz(伤害[2]*0.8)
		if 倍率 ==9999 then
		c[1] =854--修改自选装备固定数值
		c[2] =733--修改自选装备固定数值
		end
	...
	elseif b == 19 then -- 帽子
		...
		c[3] = qz(qsz(防御[1],防御[2]*shangxian)*倍率)--qz(防御[2]*0.8)--取随机数(防御[1],防御[2])--
		c[4] = qz(qsz(魔法[1],魔法[2]*shangxian)*倍率)--+100000
		if 倍率 ==9999 then
		c[3] =126--修改自选装备固定数值
		c[4] =246--修改自选装备固定数值
		end
	...
	elseif b == 20 then -- 项链
		...
		c[5] = qz(qsz(灵力[1],灵力[2]*shangxian)*倍率)--qz(灵力[2]*0.8)--取随机数(灵力[1],灵力[2])--
		if 倍率 ==9999 then
		c[5] =294--修改自选装备固定数值
		end
	...
	elseif b == 22 then -- 腰带
		...
		c[3] = qz(qsz(防御[1],防御[2]*shangxian)*倍率)--qz(防御[2]*0.8)--取随机数(防御[1],防御[2])--
		if a >0 then
			c[6] = qz(qsz(气血[1],气血[2]*shangxian)*倍率)--qz(气血[2]*0.8)+100000--取随机数(气血[1],气血[2])--
		end
		if 倍率 ==9999 then
		c[3] =126--修改自选装备固定数值
		c[6] =494--修改自选装备固定数值
	...
	elseif b == 23 then -- 鞋子
		...
		c[3] = qz(qsz(防御[1],防御[2]*shangxian)*倍率)--qz(防御[2]*0.8)--取随机数(防御[1],防御[2])--
		c[7] = qz(qsz(敏捷[1],敏捷[2]*shangxian)*倍率)--qz(敏捷[2]*0.8)--取随机数(敏捷[1],敏捷[2])--
		if 倍率 ==9999 then
		c[3] =126--修改自选装备固定数值
		c[7] =77--修改自选装备固定数值
		end
...
function 装备处理类:取160装备属性(制造种类,元身属性,id,打造id)
	...
	if 制造种类 < 19 then -- 武器
		打造属性[1] = qsz(元身属性.命中*1.08, 元身属性.命中*shangxian)
		打造属性[2] = qsz(元身属性.伤害*1.08, 元身属性.伤害*shangxian)-1
		local 属性个数 = 0
		local 选取属性 = 0
	...
	elseif 制造种类 == 19 or 制造种类 == 20 then -- 帽子
		打造属性[3] = qsz(元身属性.防御*1.08, 元身属性.防御*shangxian)
		打造属性[4] = qsz(元身属性.魔法*1.08, 元身属性.魔法*shangxian)
	elseif 制造种类 == 21 then -- 项链
		打造属性[5] = qsz(元身属性.灵力*1.08, 元身属性.灵力*shangxian)
	elseif 制造种类 == 22 or 制造种类 == 23 then -- 衣服
		打造属性[3] = qsz(元身属性.防御*1.08, 元身属性.防御*shangxian)
		local 属性个数 = 0
		local 选取属性 = 0
	...
	elseif 制造种类 == 24 then -- 腰带
		打造属性[3] = qsz(元身属性.防御*1.08, 元身属性.防御*shangxian)
		打造属性[6] = qsz(元身属性.气血*1.08, 元身属性.气血*shangxian)
	elseif 制造种类 == 25 then -- 鞋子
		打造属性[3] = qsz(元身属性.防御*1.08, 元身属性.防御*shangxian)
		打造属性[7] = qsz(元身属性.敏捷*1.08, 元身属性.敏捷*shangxian)+1
	end
