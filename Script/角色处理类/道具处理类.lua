            ...
            if 仓库[页数] then
                ...
                for 格子 = 1, 仓库.总数 do
                    ...
                    if 道具 then  -- 存在道具时处理
                        ...
                        -- 1. 处理50-80级非专用装备（总类=2为装备）
                        if 道具.总类 == 2 and 道具.等级 and 道具.等级 >= 50 and 道具.等级 <= 80 then
                            if not 道具.专用 and not 道具.灵饰 and not 道具.召唤兽装备 then
...
function 道具处理类:取回收环价格By道具(道具)
    -- 这里复用原取回收环价格的计算逻辑，示例：
    return 道具.等级 * 1000  -- 实际替换为原函数的具体计算方式
end
    ...
    for n=1,80 do
        if 玩家数据[数字id].角色.道具[n] and self.数据[玩家数据[数字id].角色.道具[n]] then
            ...
            if self.数据[道具id] then
                ...
                if  self.数据[道具id].总类==2 and self.数据[道具id].等级 and self.数据[道具id].等级>=50 and self.数据[道具id].等级<=80 then
                    if not self.数据[道具id].专用 and not self.数据[道具id].灵饰 and not self.数据[道具id].召唤兽装备 then
...
-------------------------------------------------------------------------------------
    ...
    elseif 名称=="月饼" then
        ...
        local 剩余月饼次数 = 100 - 玩家数据[id].角色.月饼
...
function 道具处理类:高级藏宝图处理(id)
    ...
    elseif 奖励参数<=20 then
        local 名称="分解符"
        玩家数据[id].道具:给予超链接道具(id,"分解符",20,nil,链接)
    ...
    elseif 奖励参数<=75 then
        设置任务102(id) --宝宝窝
        常规提示(id,"#Y你用力铲下去，由于用力过猛，整个身子差点倒下去，好像碰坏了什么建筑...#56")
    elseif 奖励参数<=85 then
