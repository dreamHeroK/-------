...
-- @Last Modified time: 2026-05-21 14:59:02
local 智能门派施法 = {}
...
智能门派施法["化生寺"] = function(self, n)
    ...
    for i = 1, #self.参战单位 do
        if i ~= n and self.参战单位[i].队伍 == 自身.队伍 then
            -- 统计死亡单位
            ...
            -- 统计低血量单位（气血<30%）
            if self.参战单位[i].气血 > 0 and
               self.参战单位[i].气血 < self.参战单位[i].最大气血 * 0.3 then
                低血量单位数 = 低血量单位数 + 1
            end
    ...
    -- 第一优先级：晶清诀（需满足条件）
    if not 通过 then
        if 自身.愤怒 and 自身.愤怒 >= 120 and 被封印单位数 >= 3 then
            -- 检查是否有晶清诀技能
            if 取技能等级(自身, "晶清诀") ~= 0 then
                self.参战单位[n].指令 = {下达=false, 类型="法术", 目标=n, 敌我=0, 参数="晶清诀", 附加=""}
                通过 = true
            end
        end
    end
    ...
    if not 通过 then
        -- 只有1-2个死亡单位时才优先复活
        if 死亡单位数 >= 1 and 死亡单位数 <= 2 and 取技能等级(自身, "我佛慈悲") ~= 0 then
            local 死亡目标 = 取死亡目标(self, n)
            if 死亡目标 then
                self.参战单位[n].指令 = {下达=false, 类型="法术", 目标=死亡目标, 敌我=0, 参数="我佛慈悲", 附加=""}
                通过 = true
            end
        end
    end

    -- 第三优先级：群体治疗 vs 单体治疗决策
    if not 通过 then
        if 低血量单位数 >= 3 and 取技能等级(自身, "推气过宫") ~= 0 then
            -- 多个单位低血量，优先群体治疗
            local 低血目标 = self:取气血百分比最低(n, 2)
            if 低血目标 and 低血目标 ~= 0 then
                self.参战单位[n].指令 = {下达=false, 类型="法术", 目标=低血目标, 敌我=0, 参数="推气过宫", 附加=""}
                通过 = true
            end
        elseif 低血量单位数 >= 1 and 低血量单位数 <= 2 and 取技能等级(自身, "推气过宫") ~= 0 then
            -- 少量单位低血量，使用单体重
            local 低血目标 = self:取气血百分比最低(n, 2)
            if 低血目标 and 低血目标 ~= 0 and
               self.参战单位[低血目标].气血 < self.参战单位[低血目标].最大气血 * 0.5 then
                self.参战单位[n].指令 = {下达=false, 类型="法术", 目标=低血目标, 敌我=0, 参数="推气过宫", 附加=""}
                通过 = true
            end
        end
