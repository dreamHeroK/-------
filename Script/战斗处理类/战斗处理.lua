...
function 战斗处理类:加载系统PK单位(单位组,位置)
	...
	for n = 1, #单位组 do
		...
		if self.参战单位[#self.参战单位].附加阵法 ~= "普通" then
			self:增加阵法属性(#self.参战单位, self.参战单位[#self.参战单位].附加阵法, 单位组[n].位置)
		end
		if 单位组[n].角色 then
...
function 战斗处理类:添加奇经八脉属性(编号, 门派)
	...
	elseif 门派 == "凌波城" then
		...
		if self:取经脉(编号,"凌波城","真君显灵") then
			...
			self.参战单位[编号].超级战意 = 50 --进入战斗自带50层超级战意，用于开启真君显灵
...
function 战斗处理类:添加内丹属性(单位, 内丹)
	...
	if ls2  then
		ls3 = ls3 + ls4 - 1
		if ls2.开天辟地 and ls2.开天辟地 ~= 0 then
			单位.伤害 = 单位.伤害 + math.floor(单位.力量 * 0.05 * ls2.开天辟地)
		end
		if ls2.破釜沉州 and ls2.破釜沉州 ~= 0 then
			单位.必杀 = 单位.必杀 + 2 * ls2.破釜沉州
		end
	end
	for i = 1, #单位.内丹.格子 do
...
--  self:还原单位属性()
	...
	if not self.PK战斗 then
		...
		elseif 逃跑 == 1 then --逃跑
			if self.任务id~=nil and 任务数据[self.任务id]~=nil then 任务数据[self.任务id].zhandou=nil end
			for n=1,#self.参战玩家 do
