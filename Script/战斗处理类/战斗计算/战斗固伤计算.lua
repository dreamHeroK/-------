...
-- @Last Modified time: 2026-02-11 23:28:28
...
function 战斗固伤计算:固伤技能计算1(编号,名称,等级,目标,目标数,执行数,伤害系数,战斗数据)
  ...
  --   elseif 名称=="同伤式" then
	...
	if 战斗数据.参战单位[编号].武器专属 and 战斗数据.参战单位[编号].武器专属.类型 == "固定伤害" and 战斗数据.参战单位[编号].武器专属.门派 == 战斗数据.参战单位[编号].门派 then
		伤害 = 伤害 * (1 + 战斗数据.参战单位[编号].武器专属.数值 / 100)
	end
	...
	elseif 伤害>=4 and 战斗数据.参战单位[目标].法术状态.修罗咒 then
		...
		if 战斗数据.参战单位[目标].法术状态.波澜不惊.次数<4 then
			...
			local 波澜不惊等级 = 战斗数据.参战单位[目标].法术状态.波澜不惊.等级 or 0
			if 伤害>(波澜不惊等级*2+150)*2 then
				伤害=(波澜不惊等级*2+150)*2
			end
			if 伤害<1 then
				伤害 = 1
	...
	-- 	if 战斗数据.参战单位[目标].磐石==nil or 战斗数据.参战单位[目标].磐石.记录组 == nil then --首个攻击自己的先记录下来
	-- 		战斗数据.参战单位[目标].磐石={记录组={}}
	...
	if 伤害类型==1 then
		local 法暴几率 = 0
		if 战斗数据.参战单位[编号].固伤暴击等级 and 战斗数据.参战单位[目标] and 战斗数据.参战单位[目标].等级 then
			local dengji = 战斗数据.参战单位[目标].等级
			if not dengji or dengji < 50 then dengji = 50 end
			法暴几率 = 战斗数据.参战单位[编号].固伤暴击等级*10/dengji
		end
		if 法暴几率 >= 取随机数(1,100) then
			伤害 = 伤害 * 2
			table.insert(特效,"暴击")
		end
	end
