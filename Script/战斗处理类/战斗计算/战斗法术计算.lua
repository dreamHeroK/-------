...
-- @Last Modified time: 2026-04-25 15:30:10
...
function 战斗法术计算:法攻循环前附加流程(编号,名称,伤害系数,战斗数据,目标数)--用在这里效率更高，因为不用循环  需要调用到挨打方的，就要在经脉结果添加了
	if 战斗数据.参战单位[编号].门派 == "化生寺" then
		if 名称 == "唧唧歪歪" then
			if 战斗数据:取经脉(编号, "化生寺","诵经") then
				...
				伤害系数 = 伤害系数 * (1+0.16*战斗数据.参战单位[编号].法术状态["诵经"].层数)
			end
		...
		if 战斗数据:取经脉(编号, "化生寺","坐禅") then
			...
			-- 伤害 = 伤害 * (1 + #fyjn*0.01)
			伤害系数 = 伤害系数 * (1 + #fyjn*0.01) ----------佬伍修改（化生寺经脉问题报错）还需测试
		end
	...
	-- 		if 名称=="雷霆万钧" and not 战斗数据:取经脉(编号, "天宫", "雷霆汹涌") then
...
--修改
	elseif 战斗数据.参战单位[编号].门派 == "天宫" then
		    if 战斗数据.参战单位[编号].法术状态.天雷灌注 then
		        if 名称 == "雷霆万钧" then
		            -- 只有不点汹涌经脉，才正常吃增伤 + 消耗判断
		            if not 战斗数据:取经脉(编号, "天宫", "雷霆汹涌") then
		                -- 50%几率不消耗（天劫经脉效果）
		                if 战斗数据:取经脉(编号, "天宫", "天劫") and math.random(1,100) <= 50 then
		                    -- 不消耗天雷，只加伤害
		                    伤害系数 = 伤害系数 * 1.25
		                else
		                    -- 正常消耗天雷 + 加伤害
...
-- ========================================================
	...
	if (战斗数据:取被动(编号,"龙魂") or 战斗数据.参战单位[编号].龙魂~=nil) and (战斗数据.参战单位[编号].法术状态==nil or 战斗数据.参战单位[编号].法术状态["龙魂"]==nil ) then
...
function 战斗法术计算:法攻技能计算1(编号,名称,等级,目标,目标数,执行数,伤害系数,战斗数据)
	...
	if 战斗数据.参战单位[编号].名称 ~= "公共世界BOOS" then
...
function 战斗法术计算:法术结算(战斗数据,shanghai,法暴倍率,编号,目标,名称,弱点,吸收,技能名称)
	local 法暴成功 = false
	local 法暴几率 = (战斗数据.参战单位[编号].法术暴击等级 - 战斗数据.参战单位[目标].抗法术暴击等级)*8/战斗数据.参战单位[目标].等级
	local 附加几率 = 0

	if 战斗数据.参战单位[编号].法术状态.超级进击法暴 then
		附加几率 = 附加几率+20
		elseif 战斗数据.参战单位[编号].法术状态.出奇制胜 then
		附加几率 = 附加几率+20
		elseif 战斗数据.参战单位[编号].法术状态.好梦连连 then
		附加几率 = 附加几率+20
	elseif 战斗数据.参战单位[编号].法术状态.高级进击法暴 then
		附加几率 = 附加几率+战斗数据.参战单位[编号].法术状态.高级进击法暴.加法暴
	elseif 战斗数据.参战单位[编号].法术状态.进击法暴 then
		附加几率 = 附加几率+战斗数据.参战单位[编号].法术状态.进击法暴.加法暴
	elseif 战斗数据.参战单位[编号].法术状态.灵彻太虚 then
		附加几率 = 附加几率 - 5
	elseif 战斗数据.参战单位[编号].法术暴击几率 then
		附加几率 = 附加几率 + 战斗数据.参战单位[编号].法术暴击几率
	end
	...
	if 战斗数据:取经脉(编号, "方寸山", "雷动") and 名称=="落雷符" and 目标==zhiling.目标 then
		附加几率 = 附加几率 + 15
	end
	if 战斗数据:取经脉(编号, "方寸山", "雷动") and 名称=="五雷正法" and 目标==zhiling.目标 then
		附加几率 = 附加几率 + 15
	end
	if 战斗数据:取经脉(编号, "方寸山", "驱雷") and 名称=="五雷咒"  then
		附加几率 = 附加几率 + 3
	end
	if 名称=="五雷正法"and  战斗数据.参战单位[编号].法术状态.雷法·坤伏.层数>0  then
	...
	if 战斗数据.参战单位[编号].法暴+法暴几率>=取随机数() then
		...
		if 战斗数据.参战单位[编号].法术暴击伤害 then
			法暴倍率 = 法暴倍率 + 战斗数据.参战单位[编号].法术暴击伤害/1000
		end


		if 战斗数据.参战单位[编号].超爆~=nil then----超级法术暴击
	...
	--------------C改奇袭
	if 战斗数据.参战单位[目标].法伤化劲 then
		shanghai = shanghai - 战斗数据.参战单位[目标].法伤化劲
	elseif 战斗数据.参战单位[目标].固伤化劲 then
		shanghai = shanghai - 战斗数据.参战单位[目标].固伤化劲
	end
	--------------C改奇袭
	if 伤害类型==1 then
	...
	if 战斗数据.参战单位[编号].好梦~=nil then
		local a=取随机数(1,100)
		if a<=20 then
		    shanghai = qz(shanghai+shanghai*0.3)
		else
		    shanghai=shanghai
		end
	end
	if 战斗数据.参战单位[编号].武器专属  and 战斗数据.参战单位[编号].武器专属.门派 == 战斗数据.参战单位[编号].门派 and 战斗数据.参战单位[编号].武器专属.类型 == "法术伤害" then
		shanghai = qz(shanghai * ((100 + 战斗数据.参战单位[编号].武器专属.数值) / 100))
	end
