...
-- @Last Modified time: 2026-06-04 18:47:58
-- @Author: baidwwy
-- @Date:   2023-09-29 08:45:32
-- @Last Modified by:   baidwwy
...
function 初始技能计算:技能消耗(战斗数据,攻击方,数值,类型,名称)
	...
	elseif 类型=="魔法和愤怒" then
		...
		elseif 名称 == "秘传三昧真火" then
			return self:消耗魔法和愤怒(战斗数据, 攻击方,  60, 20, 名称)
		elseif 名称 == "诸天看护" then
			return self:消耗魔法和愤怒(战斗数据,攻击方, 100, 10,名称)
		elseif 名称 == "秘传飞砂走石" then
			return self:消耗魔法和愤怒(战斗数据,攻击方, 120, 20,名称)
		end
		return false
	elseif 类型=="木精" then
		if 名称 == "古藤秘咒" then
...
if not 战斗数据.PK战斗 and 攻击方.本命法宝 and 攻击方.本命法宝.开启 and 攻击方.本命法宝.耐久>0 then
		if  攻击方.本命法宝.名称=="无尘雪" and 取随机数(1,100) <= 攻击方.本命法宝.等级*1+4  then
		sl=0
		end
	end
	if 攻击方.战意 < sl then
...
function 初始技能计算:取使用前CD(战斗数据,攻击方,名称)
	...
	elseif 名称=="救死扶伤" and 战斗数据:取经脉(攻击方.序号, "化生寺", "心韧") then
	    return 4
	elseif 名称=="亢龙归海" or 名称=="魔焰滔天"  or 名称=="威仪九霄"or 名称=="风卷残云" then
	    return 3
    ...
    elseif 名称=="秘传三昧真火" or 名称=="秘传飞砂走石"  then
	    ...
	    return 4
	elseif 名称=="侵蚀·嗔怒金刚·刻骨"or 名称 == "侵蚀·嗔怒金刚·钻心"or 名称 == "侵蚀·嗔怒金刚·噬魂" or 名称=="侵蚀·唧唧歪歪·刻骨"or 名称 == "侵蚀·唧唧歪歪·钻心"or 名称 == "侵蚀·唧唧歪歪·噬魂"then
	    return 150
...
function 初始技能计算:取使用后CD(战斗数据 , 攻击方, 名称)
	...
	elseif  名称 == "救死扶伤1" then
		return 3
	elseif 名称=="观照万象" then
    		return 全局基础数据.观照剩余冷却
	elseif 名称=="长驱直入" or 名称 == "魔火焚世"or 名称 == "月下霓裳" or 名称 == "疯狂鹰击" or 名称=="翩鸿一击" or 名称=="亢龙归海" or 名称=="魔焰滔天"  or 名称=="顺势而为" or 名称=="威仪九霄"or 名称=="风卷残云" then
		return 6
	elseif 名称=="侵蚀·嗔怒金刚·刻骨"or 名称 == "侵蚀·嗔怒金刚·钻心"or 名称 == "侵蚀·嗔怒金刚·噬魂" or 名称=="侵蚀·唧唧歪歪·刻骨"or 名称 == "侵蚀·唧唧歪歪·钻心"or 名称 == "侵蚀·唧唧歪歪·噬魂"then
	    return 150
	elseif 名称=="诸天看护" or  名称=="谆谆教诲" or 名称=="潜龙在渊" or 名称=="钟馗论道" or 名称=="偷龙转凤"	or 名称=="牵魂蛛丝" or 名称=="噬毒" or 名称=="困兽之斗" or 名称=="真君显灵" or 名称=="齐天神通" then
	elseif 名称=="风云变色"  then
		return 4
	elseif 名称== "秘传三昧真火" or  名称== "秘传飞砂走石"then
    	if   战斗数据:取经脉(攻击方.序号, "魔王寨", "固基") then
	...
	elseif 名称=="天眼神通"  then
	    ...
	    return 8
    elseif 名称=="碎玉弄影" or 名称=="画地为牢" or 名称=="凋零之歌" then
    	return 5
	elseif 名称=="醍醐灌顶" or  名称 == "风火燎原"  then
	    return 12
	elseif 名称=="妙悟" then
	    return 2
    ...
    elseif 名称=="判官令" and 战斗数据:取经脉(攻击方.序号, "阴曹地府", "咒令") then
	    return 6
	elseif 名称=="渡劫金身" or 名称=="鸿渐于陆" or 名称=="落花成泥" or 名称=="天命剑法" or 名称 == "同舟共济" or 名称 == "妖风四起" or 名称 == "魍魉追魂" then
	    return 4
	elseif 名称=="功德无量" then
	    return 12
	elseif 名称=="电光火石" then
    ...
    elseif 名称== "花谢花飞" then
    	return 16
	elseif 名称=="绝命毒牙" or 名称 == "绝处逢生" or 名称 == "天神怒斩" or 名称=="披挂上阵" then
	    return 10
...
function 初始技能计算:封印技能(名称) --,"离魂符" ,"金刚镯"
	-- local 临时名称={"摧心术","反间之计","催眠符","失心符","落魄符","失忆符","追魂符","失魂符","定身符","莲步轻舞","如花解语","似玉生香","碎玉弄影",
	-- "娉婷袅娜","错乱","百万神兵","日月乾坤","威慑","含情脉脉","魔音摄魂","夺魄令","惊魂掌","煞气诀","秘传封印","顺势而为","妖风四起","偷龙转凤"}
	-- for n=1,#临时名称 do
	-- 	if 临时名称[n]==名称 then return true end
	-- end
	if Qu封印技能[名称] then
...
function 初始技能计算:群体封印技能(名称)
	-- local 临时名称={"毁灭之光","一笑倾城","镇妖","掌心雷","八戒上身"}
	-- for n=1,#临时名称 do
	-- 	if 临时名称[n]==名称 then return true end
	-- end
	if Qu群体封印技能[名称] then
...
function 初始技能计算:减益技能(名称)
	-- local 临时名称={"尸腐毒","紧箍咒","放下屠刀","碎甲术","破甲术","河东狮吼","雾杀","笑里藏刀","锢魂术","月下霓裳","否极泰来","尸腐无常","碎甲符","凋零之歌",
	-- "魂飞魄散","落花成泥","雷浪穿云","知己知彼","画地为牢","噬毒","凝滞术","停陷术","死亡之音"}
	-- for n=1,#临时名称 do
	-- 	if 临时名称[n]==名称 then return true end
	-- end
	if Qu减益技能[名称] then
...
function 初始技能计算:增益技能(名称) --,"达摩护体" ,"天神护法"
	-- local 临时名称={"变身","移魂化骨","狂怒","后发制人","杀气诀","安神诀","分身术","金刚护法","潜龙在渊","天雷灌注","雷怒霆激","霹雳弦惊",
	-- "金刚护体","韦陀护法","一苇渡江","佛法无边","楚楚可怜","乘风破浪","神龙摆尾","生命之泉","炼气化神","天地同寿","乾坤妙法","普渡众生",
	-- "灵动九天","幽冥鬼眼","修罗隐身","火甲术","魔王回首","定心术","极度疯狂","魔息术","天魔解体","盘丝阵","不动如山","碎星诀","镇魂诀","明光宝烛",
	-- "金身舍利","炎护","蜜润","法术防御","太极护法","罗汉金钟","流云诀","啸风诀","野兽之力","魔兽之印","光辉之甲","圣灵之甲","牛劲","毒萃","波澜不惊",
	-- "其疾如风","其徐如林","侵掠如火","岿然如山","龙战于野","花语歌谣","无双战魂","无间地狱","媚眼如丝","清静菩提","鸣雷诀","逆鳞","钟馗论道","诸天看护",
	-- "渡劫金身","天神护体","凝神术","颠倒五行","莲心剑意","剑意莲心","幻镜术","同舟共济","真君显灵","无畏布施","北冥之渊",
	-- "功德无量","森罗迷瘴","心随意动","燃血术","化羽为血","气慑天军","九幽除名","铜头铁臂","无所遁形","呼子唤孙","齐天神通","金刚不坏","灵能激发",
	-- "修罗咒","身似菩提","菩提心佑","心如明镜","法力陷阱"}
	-- for n=1,#临时名称 do
	-- 	if 临时名称[n]==名称 then return true end
	-- end
	if Qu增益技能[名称] then
...
function 初始技能计算:取吸血异常(战斗数据,编号)
	-- local 临时名称= {"日月乾坤", "毒", "尸腐毒", "雾杀", "威慑", "反间之计", "百万神兵", "错乱", "镇妖", "催眠符", "失心符", "落魄符", "失忆符", "追魂符",
	--  "失魂符", "定身符", "莲步轻舞", "如花解语", "似玉生香", "含情脉脉", "魔音摄魂", "天罗地网", "夺魄令", "惊魂掌", "摧心术","雷浪穿云"}
...
function 初始技能计算:不可复活(单位)
	-- local 临时名称= {"魔音摄魂", "死亡召唤", "锢魂术", "秘术",  "困兽"}
...
function 初始技能计算:取是否被封印(战斗数据,编号)
	-- local 临时名称 = {"催眠符", "失心符", "落魄符", "失忆符", "追魂符","失魂符", "定身符", "莲步轻舞", "如花解语", "似玉生香",
	--  "娉婷袅娜", "镇妖", "错乱", "百万神兵", "日月乾坤", "威慑", "含情脉脉", "魔音摄魂", "夺魄令", "惊魂掌", "煞气诀", "毁灭之光", "秘传封印"
	--  ,"顺势而为","妖风四起"}
	for i, v in pairs(战斗数据.参战单位[编号].法术状态) do
...
function 初始技能计算:取攻击状态(战斗数据,编号)
	-- 调试信息(编号.."号可执行攻击动作")
	-- local 技能名称 = {"煞气诀", "象形", "冰冻", "横扫千军","惊天动地", "破釜沉舟", "鹰击", "催眠符", "追魂符", "定身符", "如花解语","慧眼",
	--  "似玉生香", "百万神兵", "日月乾坤", "威慑","连环击", "含情脉脉", "毁灭之光", "秘传封印", "烈火焚原","顺势而为","夺魄令"}
	-- for n = 1, #技能名称 do
	-- 	if 战斗数据.参战单位[编号].法术状态[技能名称[n]] ~= nil then
	-- 		if 技能名称[n]=="夺魄令" and 战斗数据.参战单位[编号].法术状态["夺魄令"].封物理 then
	-- 			return true
	-- 		end
	-- 		return false
	-- 	end
...
function 初始技能计算:取法术状态(战斗数据,编号)
	--调试信息(编号.."号可执行法术动作")
	-- local 技能名称 = {"煞气诀", "夺魄令", "修罗隐身", "冰冻", "象形", "象形（休息）", "横扫千军","惊天动地", "破釜沉舟", "鹰击", "催眠符", "失心符", "落魄符", "失魂符", "莲步轻舞",
	-- "似玉生香", "错乱", "日月乾坤", "威慑", "含情脉脉","连环击", "发瘟匣", "毁灭之光", "秘传封印","顺势而为","一笑倾城","慧眼","血雨"
	-- ,"妖风四起"}
	-- for n = 1, #技能名称 do
		-- if 战斗数据.参战单位[编号].法术状态[技能名称[n]] ~= nil then
		-- 	return false
...
function 初始技能计算:取特技状态(战斗数据,编号)
	--调试信息(编号.."号可执行法术动作")
	-- local 技能名称 = {"煞气诀", "娉婷袅娜", "横扫千军","连环击","惊天动地", "破釜沉舟", "鹰击", "催眠符", "失忆符", "日月乾坤", "烈火焚原","顺势而为","镇妖", "象形"}
	-- for n = 1, #技能名称 do
	-- 	if 战斗数据.参战单位[编号].法术状态[技能名称[n]] ~= nil then
	-- 		return false
	-- 	end
...
function 初始技能计算:取异常状态法术(法术状态)
	-- return {"日月乾坤", "毒", "尸腐毒", "雾杀", "威慑", "反间之计", "百万神兵", "错乱", "镇妖", "催眠符", "失心符", "落魄符", "失忆符", "追魂符",
	--  "失魂符", "定身符", "莲步轻舞", "如花解语","连环击", "似玉生香", "含情脉脉", "魔音摄魂", "天罗地网", "夺魄令", "惊魂掌", "摧心术","雷浪穿云"}
	local fszt={}
...
function 初始技能计算:取法宝增益状态()
	return{"干将莫邪", "苍白纸人", "五彩娃娃", "混元伞","乾坤玄火塔","分水","赤焰","天煞","神木宝鼎","九梵清莲","苍灵雪羽","烽火狼烟","璞玉灵钵","金蟾"}
end
...
function 初始技能计算:取技能所属门派(名称)
	...
	elseif 名称=="勾魂" then
		return "盘丝洞"
	-- elseif 名称 == "三荒尽灭" or 名称 == "三荒尽灭" or 名称 == "一斧开天" or 名称 == "铁火双杨" or 名称 == "铁血生风" or 名称 == "力劈苍穹" or 名称 == "铁风撼盾" or 名称 == "枫魂汲魄" then
	-- 	return "九黎城"
...
function 初始技能计算:魔法消耗(战斗数据,攻击方, 数值, 名称)
	...
	-- if not self:战意消耗(战斗数据,攻击方,名称) then
	-- 	return false
	-- end
...
function 初始技能计算:魔法和木精消耗(战斗数据,攻击方, 魔法, 木精, 名称)
	...
	if 攻击方.魔法 < 魔法消耗 or not 攻击方.法术状态.木精 then
		return false
	else
...
function 初始技能计算:取行动状态(战斗数据,编号)
	...
	-- 修复后：
	if (战斗数据.参战单位[编号].气血 <= 0 or 战斗数据.参战单位[编号].捕捉 or 战斗数据.参战单位[编号].逃跑 or 战斗数据.参战单位[编号].死亡击飞) and not 战斗数据.参战单位[编号].狂袭 then
		return false
	elseif 战斗数据.参战单位[编号].法术状态.横扫千军 ~= nil and 战斗数据.参战单位[编号].法术状态.横扫千军.回合==1 then
		return false
	elseif 战斗数据.参战单位[编号].法术状态.幸运一击 ~= nil and 战斗数据.参战单位[编号].法术状态.幸运一击.回合==1 then
		return false
	...
	elseif 战斗数据.参战单位[编号].法术状态.催眠符 ~= nil then
		...
		return false
	elseif 战斗数据.参战单位[编号].法术状态.楚楚可怜 ~= nil and (战斗数据.参战单位[编号].指令 or {}).类型~="捕捉" and (战斗数据.参战单位[编号].指令 or {}).类型~="召唤" and  (战斗数据.参战单位[编号].指令 or {}).类型~="道具" and  (战斗数据.参战单位[编号].指令 or {}).类型~="保护" and  (战斗数据.参战单位[编号].指令 or {}).类型~="防御"  and  (战斗数据.参战单位[编号].指令 or {}).类型~="逃跑" then
		return false
	elseif 战斗数据.参战单位[编号].法术状态.冰川怒 ~= nil then
		return false
	elseif 战斗数据.参战单位[编号].法术状态.亢龙归海 ~= nil  then --and 取随机数() < 35
		return false
