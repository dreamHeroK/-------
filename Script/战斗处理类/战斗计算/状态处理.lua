...
-- @Date:   2023-10-11 14:46:42
-- @Last Modified by:   baidwwy
-- @Last Modified time: 2023-10-12 20:14:18
-- @Author: baidwwy
-- @Date:   2023-08-31 23:34:34
-- @Last Modified by:   baidwwy
-- @Last Modified time: 2023-10-12 19:01:36
-- @作者: baidwwy
-- @邮箱:  313738139@qq.com
-- @创建时间:   2021-03-04 18:32:15
-- @最后修改来自: baidwwy
-- @Last Modified time: 2023-07-03 04:41:43
local 状态处理 = class()
...
function 状态处理:添加状态(战斗数据,名称,挨打方,攻击方,等级,境界,指定,套装状态)
	...
	if zza and zza.状态属性 then
		...
		-- 常规提示(攻击方.玩家id,名称.." 未录入脚本，请截图告知GM")
	end
	...
	elseif 名称 == "金刚护体" then
		...
		if 攻击方.类型 == "角色" or 攻击方.类型 == "系统角色" then
			防御 = qz(等级 * 2.25) --人物增强50%
		else
			防御 = qz(等级 * 1.5)
		end
	...
	elseif 名称 == "金刚护法" then
		...
		if 攻击方.类型 == "角色" or 攻击方.类型 == "系统角色" then
			伤害 = qz(等级*1.95)   --搜出来大概是1.3，人物增强50%
		else
			伤害 = qz(等级*1.3)
		end
	...
	elseif 名称 == "剑意莲心" then
		if 战斗数据:取经脉(攻击编号, "普陀山", "莲音") and sj()<=50 then
	...
	elseif 名称 == "手到擒来" or 名称 == "半济而击" or 名称 == "变化莫测" or 名称 == "神谋魔道" or 名称 == "言出法随" or 名称 == "撼海狂龙" or 名称 == "涤瑕蹈隙" or 名称 == "八攻八克"
			or 名称 == "避实击虚" or 名称 == "力大无穷" or 名称 == "自在逍遥" or 名称 == "月明如昼" or 名称 == "自我疗愈" or 名称 == "兵贵神速" or 名称 == "护体如磐"
			or 名称 == "以牙还牙" or 名称 == "洞察一切" or 名称 == "坚如磐石" or 名称 == "鹰击长空" or 名称 == "除邪惩恶" or 名称 == "妖魔鬼怪" or 名称 == "鸿运当头"
			or 名称 == "后发制人" or 名称 == "苦思冥想"  then
		回合=150
	elseif 名称 == "瘴气" then
	...
	elseif 名称 == "幽冥鬼眼" then
		...
		if not 战斗数据.PK战斗 and 战斗数据.参战单位[攻击编号].本命法宝 and 战斗数据.参战单位[攻击编号].本命法宝.开启 and 战斗数据.参战单位[攻击编号].本命法宝.耐久>0 then
			if  战斗数据.参战单位[攻击编号].本命法宝.名称=="祭魄印" and  战斗数据.参战单位[攻击编号].门派=="阴曹地府" and 取随机数(1,100) <= 战斗数据.参战单位[攻击编号].本命法宝.等级  then
			伤害 = qz(伤害 * 1.15)
			法伤 = qz(法伤 * 1.15)
			end
		end
	elseif 名称 == "灵动九天" then
		if not 战斗数据.PK战斗 and 战斗数据.参战单位[攻击编号].本命法宝 and 战斗数据.参战单位[攻击编号].本命法宝.开启 and 战斗数据.参战单位[攻击编号].本命法宝.耐久>0 then
			if  战斗数据.参战单位[攻击编号].本命法宝.名称=="琼华玉露" and  战斗数据.参战单位[攻击编号].门派=="普陀山" and 取随机数(1,100) <= 战斗数据.参战单位[攻击编号].本命法宝.等级  then
			速度 = qz(速度 * 1.2)
			防御 = qz(防御 * 1.2)
			end
		end
...
--新增五庄观本命法宝属性
	if not 战斗数据.PK战斗 and 战斗数据.参战单位[攻击编号].本命法宝 and 战斗数据.参战单位[攻击编号].本命法宝.开启 and 战斗数据.参战单位[攻击编号].本命法宝.耐久>0 then
		if  战斗数据.参战单位[攻击编号].本命法宝.名称=="崆峒印" and  战斗数据.参战单位[攻击编号].门派=="五庄观" and 取随机数(1,100) <= 战斗数据.参战单位[攻击编号].本命法宝.等级  then
			-- 挨打方.防御 = 挨打方.防御 - 挨打方.防御 * 0.3
			-- 挨打方.法防 = 挨打方.法防 - 挨打方.法防 * 0.3
			战斗数据:添加状态("易伤",挨打方,攻击方,69)
		end
	end
	...
	elseif 名称 == "明光宝烛" then
		...
		if not 战斗数据.PK战斗 and 战斗数据.参战单位[攻击编号].本命法宝 and 战斗数据.参战单位[攻击编号].本命法宝.开启 and 战斗数据.参战单位[攻击编号].本命法宝.耐久>0 then
			if  战斗数据.参战单位[攻击编号].本命法宝.名称=="炼妖壶" and  战斗数据.参战单位[攻击编号].门派=="无底洞" and 取随机数(1,100) <= 战斗数据.参战单位[攻击编号].本命法宝.等级  then
			挨打方.法术状态[名称].加防 = qz(挨打方.法术状态[名称].加防*(1+(战斗数据.参战单位[攻击编号].本命法宝.等级/100)))
			end
		end
	elseif 名称 == "金身舍利" then
		挨打方.法术状态[名称].加防 = 等级
...
-----------------------凌波城-----------------------
	...
	if 战斗数据.战斗流程[#战斗数据.战斗流程] ~= nil then
		...
		战斗数据.战斗流程[#战斗数据.战斗流程].状态处理添加[#战斗数据.战斗流程[#战斗数据.战斗流程].状态处理添加 + 1] = {编号 = 挨打方.序号, 名称 = 名称,回合=挨打方.法术状态[名称].回合,颜色=挨打方.法术状态[名称].颜色,层数=挨打方.法术状态[名称].层数,负面状态=挨打方.法术状态[名称].负面}
	end
end
