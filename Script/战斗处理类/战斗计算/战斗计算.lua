...
-- @Last Modified time: 2026-04-27 16:48:30
...
function 战斗计算:删除并重置技能属性(战斗数据,单位,zq,jh)
	...
	elseif zq == "超级否定信仰" then
		...
		-- 清除所有超级技能失效标记
		for k,v in pairs(单位) do
			if type(k) == "string" and string.find(k, "超否失效_") then
				单位[k] = nil
			end
		end
	...
	-------------- 添加超级属性
	-- 检查该超级技能是否被超级否定信仰失效
	if jh and 单位["超否失效_" .. jh] then
		-- 技能被失效，只添加标记但不应用效果
		if 单位.已加技能 ~= nil then
			单位.已加技能[#单位.已加技能 + 1] = jh
		end
		return
	end
	...
	-- 	战斗数据:添加超级否定信仰属性(单位)
	elseif jh == "超级否定信仰" then
		单位.信仰 = 1 ----------佬伍修改（修复赐福生效的超级技能也可以增加属性）
		单位.超信仰 = true
		单位.超信清 = {} --记录清除敌方信仰类技能
		单位.超信增 = {}
		单位.已加技能[#单位.已加技能 + 1] = "超级否定信仰" ----------佬伍修改（修复赐福生效的超级技能也可以增加属性）
		战斗数据:添加超级否定信仰属性(单位)
...
function 战斗计算:添加护盾(战斗数据,目标,num,类型)
	if #战斗数据.战斗流程==0 then
	    战斗数据.战斗流程[1]={}
	end
...
function 战斗计算:物理技能掉血计算(战斗数据,编号,num,目标,名称)
	...
	elseif 名称=="普通攻击" then
		if 战斗数据.参战单位[目标].法术状态.火甲术 then
			...
			-- (战斗数据,shanghai,法暴倍率,编号,目标,名称,弱点,吸收,技能名称)
			local 伤害,伤害类型 = 战斗法术计算:法术结算(战斗数据,shangh,法暴倍率,目标,编号,名称,nil)
...
function 战斗计算:法术技能掉血计算(战斗数据,编号,num,目标,名称)
	...
	if 施法流程 then
		-- (战斗数据,shanghai,法暴倍率,编号,目标,名称,弱点,吸收,技能名称)
		local 伤害,伤害类型 = 战斗法术计算:法术结算(战斗数据,shangh,法暴倍率,掉血编号,掉血目标,名称,nil)
...
function 战斗计算:封印命中率(战斗数据,名称,等级,攻击方,挨打方) --写完全部重新 检查一遍
	...
	if 攻击方.法术修炼 == nil then 攻击方.法术修炼 = 0 end
	if 挨打方.法术修炼 == nil then 挨打方.法术修炼 = 0 end
    ...
    if 攻击方.队伍 ~=0 then --直接把怪的流程给排除
	    ...
	    elseif 攻击方.门派 == "五庄观" then
	    	if 战斗数据:取经脉(攻击方.序号, "五庄观", "存思") and 名称 == "日月乾坤" and  战斗数据.回合数/3==qz(战斗数据.回合数/3) then
	    		命中等级 = 命中等级 + 150
	    	end
	...
	if 挨打方.法术状态.宁心道符 then
		抵抗基础 = qz(抵抗基础 + 挨打方.法术状态.宁心道符.境界)
	end

	if 攻击方.法术状态.相思寒针 then
		封印基础 = qz(封印基础 - 攻击方.法术状态.相思寒针.境界)
	end

    --计算公式
	local 修炼差   = qz(攻击方.法术修炼*150/攻击方.等级) - qz(挨打方.法术修炼*150/挨打方.等级) --69=19,175=21
	local 命中转换 = qz(命中等级*10/(攻击方.等级+25) - 抗封等级*8/(挨打方.等级+25))  --154 点封印命中 69=16 129 = 10 --到时候去角色里面改
    	local 几率 = 40 + 封印基础 - 抵抗基础 + 修炼差 + qz(命中等级*8/(攻击方.等级+25)-抗封等级*10/(挨打方.等级+25)) --同理得出 物理和法暴概率 = 物理宝鸡等级*10/攻击方.等级
	...
	if not 战斗数据.PK战斗 and 攻击方.本命法宝 and 攻击方.本命法宝.开启 and 攻击方.本命法宝.耐久>0 then
		if  攻击方.本命法宝.名称=="迷情镜" and 名称=="含情脉脉" and  攻击方.门派=="盘丝洞"   then
		几率 = 几率+攻击方.本命法宝.等级
		end
	end
	return 几率
...
function 战斗计算:封印命中计算(战斗数据,名称,等级,攻击方,挨打方)
	...
	成功几率=成功几率+ZJ-JS
	if 攻击方 and 攻击方.武器专属 and  攻击方.武器专属.门派 == 攻击方.门派 and 攻击方.武器专属.类型 == "封印命中" then
		成功几率 = 成功几率 + 攻击方.武器专属.数值
	end
...
function 战斗计算:减少气血(战斗数据,挨打,num,攻击,名称,友伤)
	...
	if 战斗数据.参战单位[挨打].法术状态.金蟾 then
		local hhqx = qz(num/30)
		战斗数据:动画加血流程(挨打,hhqx,"金蟾")
	end
	...
	if 友伤==nil and 战斗数据.参战单位[挨打].护体灵盾 ~= nil and 战斗数据.参战单位[挨打].护体灵盾 ~= 0 and 名称~="致命" then
		if 战斗数据.参战单位[挨打].护体灵盾 >= num then --是否要等于
			战斗数据.参战单位[挨打].护体灵盾 = 战斗数据.参战单位[挨打].护体灵盾 - num
		else
			战斗数据.参战单位[挨打].护体灵盾 = nil
		end
	end
	...
	if 战斗数据.战斗类型==100136  and 战斗数据.参战单位[挨打].队伍 == 0 then
	    if 世界挑战.气血.当前 > 0 then
	      世界挑战.气血.当前 = 世界挑战.气血.当前- math.floor(num)
	      公共世界BOOS:刷新世界排行(战斗数据.进入战斗玩家id,num)
	    end
	    if 世界挑战.气血.当前 <= 0 then
	      公共世界BOOS:结束世界挑战(战斗数据.进入战斗玩家id, num)
	    end
	end
	...
	if 战斗数据.参战单位[挨打].气血 <= 0 then --死亡判断 --死亡过多会陷入循环 就很烦
		if 战斗数据.参战单位[挨打].我佛慈悲护体 == 战斗数据.回合数 then --我佛慈悲复活后本回合内免死一次
			战斗数据.参战单位[挨打].气血 = 1
			战斗数据.参战单位[挨打].我佛慈悲护体 = nil
			return
		end
		战斗数据.执行等待 = 战斗数据.执行等待 + 5
		战斗数据.参战单位[挨打].气血 = 0
		if 战斗数据.参战单位[挨打].标记=="集火" then --集火目标死亡，自动切换下一个存活目标
			战斗数据.参战单位[挨打].标记 = nil
			for n=1,#战斗数据.参战单位 do
				if 战斗数据.参战单位[n].自动集火 then
					战斗数据:刷新自动集火目标(n)
					break
				end
			end
		end
		if 战斗数据.参战单位[挨打].类型 == 'bb' then--九黎
		...
		if 战斗数据.战斗类型 == 120028 and 战斗数据.参战单位[挨打].队伍==0 then
		    if 战斗数据.累计击杀 == nil then
		    	战斗数据.累计击杀 = 0
		    end
		    战斗数据.累计击杀 = 战斗数据.累计击杀 + 1
	  	end
