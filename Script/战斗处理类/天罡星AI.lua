...
function 天罡星AI:智能战斗(this,编号)
	...
	攻击方.指令.附加状态=nil
	攻击方.指令.固定伤害数额= nil
	if this.参战单位[编号].气血>0 then
		if this:NPC_AI取友方被封印数量(编号) >= 4 and not this.参战单位[编号].法术状态.催眠符 and not this.参战单位[编号].法术状态.镇妖 and not this.参战单位[编号].法术状态.日月乾坤 then    --被封印3个单位以上 会晶清
			this.参战单位[编号].指令.类型="法术"
			this.参战单位[编号].指令.参数= "晶清诀"
			this.参战单位[编号].指令.目标 =  self:取防御最低目标(this,this.参战单位[编号].队伍,2,编号)
			return true
		end
	end
...
function 天罡星AI:小怪处理(this,编号)
	...
	if  攻击方.门派 == "方寸山" or 攻击方.门派 == "五庄观" or 攻击方.门派 == "盘丝洞" then
		if this:NPC_AI取友方被封印数量(编号) > 3 and not 攻击方.法术状态.催眠符 and not 攻击方.法术状态.镇妖 and not 攻击方.法术状态.日月乾坤 then    --被封印3个单位以上 会晶清
			攻击方.指令.类型="法术"
			攻击方.指令.参数= "晶清诀"
			攻击方.指令.目标=  self:取防御最低目标(this,攻击方.队伍,2,编号)
			return
