...
-- @Last Modified time: 2025-12-18 02:21:46
...
function 商会处理类:数据处理(id,序号,内容) --暂时注释
	...
	elseif 序号 ==6118 then
		self:加入名店夹(数字id,内容.店主id)
	end
end

function 商会处理类:加载商会()
	self.数据={}--sj
	if f函数.文件是否存在(程序目录..[[shsj]])==false then
		os.execute("md "..[[shsj]])
	end
	local 取出文件 = 取文件的所有名 (程序目录..[[\shsj\]])
	for k,v in pairs(取出文件) do
		self.数据[k]=table.loadstring(读入文件([[shsj/]]..k..[[.txt]]))
	end
end

function 商会处理类:加入名店夹(id,店主id)
	if 玩家数据[id].角色.名店夹==nil then
		玩家数据[id].角色.名店夹={}
	end
	if 店主id then
		local qxidbh=false
		local go=true
		local gob=false
		for an=1,#玩家数据[id].角色.名店夹 do
			if 玩家数据[id].角色.名店夹[an]~=nil and 玩家数据[id].角色.名店夹[an]==店主id then
				go=false
				qxidbh=an
				break
			end
		...
		for an=1,#self.数据 do
			if self.数据[an]~=nil and self.数据[an].店主id==店主id then
				gob=true
			end
        end
		if go and gob then
			table.insert(玩家数据[id].角色.名店夹,店主id)
			发送数据(玩家数据[id].连接id,6117,玩家数据[id].角色.名店夹)
			常规提示(id,店主id .."#Y/加入名店夹成功")
		elseif not gob then
			常规提示(id,"#Y/商会数据错误")
		elseif qxidbh then
			table.remove(玩家数据[id].角色.名店夹,qxidbh)
			发送数据(玩家数据[id].连接id,6117,玩家数据[id].角色.名店夹)
			常规提示(id,店主id .."#Y/已从名店夹中删除")
		end
	end
end
...
function 商会处理类:保存数据()
	for k,v in pairs(self.数据) do
		写出文件([[shsj/]]..k..[[.txt]],table.tostring(v))
	end
end
...
function 商会处理类:购买物品(id,数据)
	...
	if self.数据[编号].物品[页数][道具].数量~= nil and self.数据[编号].物品[页数][道具].数量 <= 0 then
		self.数据[编号].物品[页数][道具].数量 = 1
	end
	if self.数据[编号].物品[页数][道具].数量~= nil and 玩家数据[id].角色:nan检测(self.数据[编号].物品[页数][道具].数量) then
		常规提示(id,"#Y/商会数据错误")
		return
	end
	if 玩家数据[id].角色:nan检测(数量) then
		常规提示(id,"#Y/商会数据错误")
		return
	end
	...
	if self.数据[编号].物品[页数][道具].可叠加 == false or self.数据[编号].物品[页数][道具].数量 == nil then
		...
		if 玩家数据[id].道具:给予道具(id,nil,数量,nil,"商会",nil,临时道具,价格,"银子","商会购买") then
			购买完成 = true
		end
...
function 商会处理类:商会取商会表名(数字id)
	...
	if 玩家数据[数字id].角色.名店夹==nil then 玩家数据[数字id].角色.名店夹={} end
	发送数据(玩家数据[数字id].连接id,6117,玩家数据[数字id].角色.名店夹)
end

function 商会处理类:商会取商会数据(数字id)
	for k,v in pairs(self.数据) do
		-- print(v.店主id,数字id,"=======================商会取商会数据")
		if v.店主id  == 数字id then
...
function 商会处理类:商会上架物品(id,数据)
	...
	if 道具id==nil then 常规提示(id,"#Y/道具数据错误，请不要移动背包物品") return end
...
function 商会处理类:商会取出物品(id,数据)
	...
	local 编号=数据.编号
	local 店主id=数据.店主id
	if 店主id ~= self.数据[编号].店主id or id~=店主id or id~=self.数据[编号].店主id then
		常规提示(id,"#Y/商会数据错误")
		return
	end
...
function 商会处理类:商会改名(id,数据)
	...
	常规提示(id,"#Y/商店名称已改为："..self.数据[数据.编号].名称)
...
function 商会处理类:商会取出资金(id,数据)
	if 数据.店主id ~= self.数据[数据.编号].店主id then
		...
		if self.数据[数据.编号].资金>0 then
			 玩家数据[id].角色:添加银子(self.数据[数据.编号].资金*0.85,"商会取出资金",1)
			 self.数据[数据.编号].资金=0
			 发送数据(玩家数据[id].连接id,6105,self.数据[数据.编号].资金*0.85)
		else
			常规提示(id,"#Y/你没有收入无法取出")
		end
end
