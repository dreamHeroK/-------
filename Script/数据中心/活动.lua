...
function 活动怪物处理(序号,任务id,玩家id)
	...
	if hdsj.等级是否变动=="是" then --这个移植到  取队伍平均等级函数里面 那么就可以删  （）等级是否变动 最低等级 最高等级（）
		...
		local fhz = qz(取队伍最高等级数(玩家数据[玩家id].队伍,玩家id))
	...
	-- if hdsj.系数 then hdsj.系数 = 0.5 end
	...
	for i=1,数量 do
		...
		if i == 1 then
			if type(hdsj.主怪编号) == "table" then
				for i=1,#hdsj.主怪编号 do
					if 怪物数据[hdsj.主怪编号[i]].模型 then
						if 任务数据[任务id].模型 == 怪物数据[hdsj.主怪编号[i]].模型 then
							sj=怪物数据[hdsj.主怪编号[i]]
							主怪序号=i
						end
					else
					    怪物数据[hdsj.主怪编号[i]].模型 = 任务数据[任务id].模型
					    怪物数据[hdsj.主怪编号[i]].名称 = 任务数据[任务id].名称
					    sj=怪物数据[hdsj.主怪编号[i]]
						主怪序号=i
					end
				end
			else
				if 怪物数据[hdsj.主怪编号].模型 == nil then
					怪物数据[hdsj.主怪编号].模型 = 任务数据[任务id].模型
					怪物数据[hdsj.主怪编号].名称 = 任务数据[任务id].名称
				end
				sj = 怪物数据[hdsj.主怪编号]
			end
		...
		if sj.角色 ~= nil then
			...
			if sj.锦衣 or (任务数据[任务id] and 任务数据[任务id].锦衣) then
				fhsj[i].锦衣 = sj.锦衣 or 任务数据[任务id].锦衣
			end
		end
		fhsj[i].伤害=qz(阶段属性[fhsj[i].等级].伤害*sj.伤害加成*(hdsj.伤害系数 or 1)*hdsj.系数)
		if sj.血量加成 > 100 then
			fhsj[i].气血 =sj.血量加成
		else
	    	fhsj[i].气血=qz(阶段属性[fhsj[i].等级].气血*sj.血量加成*(hdsj.气血系数 or 1)*hdsj.系数)
		end
		fhsj[i].灵力=qz(阶段属性[fhsj[i].等级].灵力*hdsj.系数)
		fhsj[i].速度=qz(阶段属性[fhsj[i].等级].速度*sj.速度加成*(hdsj.速度系数 or 1)*hdsj.系数)
		fhsj[i].防御=qz(阶段属性[fhsj[i].等级].防御*sj.防御加成*(hdsj.防御系数 or 1)*hdsj.系数)
		fhsj[i].法防=qz(阶段属性[fhsj[i].等级].法防*sj.法防加成*(hdsj.法防系数 or 1)*hdsj.系数)
		fhsj[i].法伤=qz(阶段属性[fhsj[i].等级].法伤*sj.法伤加成*(hdsj.法伤系数 or 1)*hdsj.系数)
		...
		fhsj[i].躲避=qz(阶段属性[fhsj[i].等级].速度*sj.速度加成*(hdsj.速度系数 or 1)*hdsj.系数)
		...
		-- fhsj[i].修炼={防修=0,攻修=0}
		-- if hdsj.攻修 == 1 then
		-- 	fhsj[i].修炼.攻修 = 阶段属性[fhsj[i].等级].修炼
		-- end
		-- if hdsj.防修 == 1 then
		-- 	fhsj[i].修炼.防修 =阶段属性[fhsj[i].等级].修炼
		-- end
		if sj.炫彩 ~= nil then
			if sj.炫彩 == "取任务" then
		...
		if sj.门派 ~= nil then
			...
			if 门派 then
				...
				-- local fhz = 取门派技能(门派)
				-- for s=1,#fhz do
				-- 	fhsj[i].被动技能[#fhsj[i].被动技能+1] =fhz[s]
				-- 	local fhz1 = 取包含技能(fhz[s])
				-- 	for n=1,#fhz1 do
				-- 		fhsj[i].主动技能[#fhsj[i].主动技能+1] =fhz1[n]
				-- 	end
				-- end
			end
		end
...
Q_随机模型={"剑侠客","逍遥生","飞燕女","英女侠","巫蛮儿","神天兵","龙太子","舞天姬","玄彩娥","虎头怪","巨魔王","狐美人","骨精灵","杀破狼","桃夭夭"}--,"桃夭夭"
Q_随机模型1={
...
Q_阵法名 = {"天覆阵","风扬阵","虎翼阵","云垂阵","鸟翔阵","地载阵","龙飞阵","鹰啸阵","蛇蟠阵"}--,"雷绝阵"
...
Q_武神坛角色佩戴法宝 = {
	大唐官府={{名称="金甲仙衣",境界=9},{名称="降魔斗篷",境界=12},{名称="风袋",境界=9},{名称="风袋",境界=9}},
...
Q_武神坛召唤兽属性={
	攻宠={伤害=2848,气血=4294,灵力=936,速度=390,防御=1085,法防=0,躲闪=597,法伤=0,魔法=1042,体质=400,魔力=199,力量=860,耐力=195,敏捷=190},
		血宠={伤害=1001,气血=10101,灵力=910,速度=401,防御=1314,法防=0,躲闪=597,法伤=0,魔法=1042,体质=1037,魔力=195,力量=321,耐力=195,敏捷=195},
		血耐={伤害=1134,气血=5637,灵力=750,速度=597,防御=2080,法防=0,躲闪=597,法伤=0,魔法=1704,体质=577,魔力=194,力量=204,耐力=638,敏捷=366},
		血攻={伤害=1932,气血=9109,灵力=821,速度=408,防御=1005,法防=0,躲闪=597,法伤=0,魔法=1520,体质=860,魔力=199,力量=400,耐力=195,敏捷=190},
		法宠={伤害=1076,气血=5181,灵力=1401,速度=375,防御=901,法防=0,躲闪=597,法伤=0,魔法=5026,体质=298,魔力=1037,力量=195,耐力=195,敏捷=190},
		法血={伤害=1200,气血=6196,灵力=910,速度=345,防御=2028,法防=0,躲闪=597,法伤=0,魔法=2150,体质=631,魔力=298,力量=216,耐力=656,敏捷=190},
	敏宠={伤害=1051,气血=4294,灵力=899,速度=1011,防御=1050,法防=0,躲闪=597,法伤=0,魔法=1042,体质=298,魔力=195,力量=195,耐力=195,敏捷=1037},
}
...
function 取角色等级武器(模型,等级)
	...
	local 角色表 = {飞燕女 = {"双短剑","环圈"},英女侠 = {"双短剑","鞭"},巫蛮儿 = {"宝珠","法杖"},偃无师 = {"巨剑","剑"},逍遥生 = {"剑","扇"},剑侠客 = {"剑","刀"},狐美人= {"爪刺","鞭"},骨精灵 = {"爪刺","魔棒"},鬼潇潇= {"伞","爪刺"},杀破狼 = {"弓弩","宝珠"},巨魔王 = {"斧钺","刀"},虎头怪 = {"斧钺","锤"},舞天姬 = {"飘带","环圈"},玄彩娥 = {"飘带","魔棒"},桃夭夭 = {"灯笼","飘带"},羽灵神 = {"弓弩","法杖"},神天兵 = {"枪矛","锤"},龙太子 = {"枪矛","扇"},影精灵 = {"爪刺","魔棒"}}
...
function 副本积分(名称)
	...
	if 名称 == "胡姬琵琶行" then--玄天残卷、修炼果 银子5W
		积分 =100
	elseif 名称 == "泾河龙王2" then--彩果、魔兽要诀 银子 九转金丹10W
		积分 =450
	...
	elseif 名称 == "乌鸡国" then -- 召唤兽装备、魔兽要诀 银子20W
		积分 =140
	elseif 名称 == "石猴授徒" then--特赦 宝石、五宝 银子10W
		积分 =70
	elseif 名称 == "猴王出世" then --五宝、修炼果 银子10W
		积分 =70
	elseif 名称 == "平顶山" then--彩果、修炼果 银子8W
		积分 =135
	elseif 名称 == "前世今生2" then--五宝、修炼果、兽决 银子30W
		积分 =300
	elseif 名称 == "齐天大圣" then--五宝、修炼果 银子10W
		积分 =110
	elseif 名称 == "金兜洞兕大王" then --修炼果 银子20W
		积分 =135
	...
	elseif 名称 == "通天河" then-- 附魔宝珠、魔兽要诀 银子8W
		积分 =120
	elseif 名称 == "西域宝藏" then -- 五色沙 --月华露、储灵袋、分解符、彩果、修炼果。c66 银子15W
		积分 =130
	elseif 名称 == "（侠）水陆大会" then --比得110-120珍珠、银子4W 概率树苗、强化石、如意丹
		积分 =130
	elseif 名称 == "红孩儿" then--玄天残卷、修炼果 银子8W
		积分 =100
	elseif 名称 == "大闹天宫" then -- 附魔宝珠、魔兽要诀 银子8W
		积分 =70
	elseif 名称 == "后裔陵墓上部" then--月华露、储灵袋、分解符、彩果、修炼果。c66 银子15W
		积分 =150
	elseif 名称 == "秘境降妖" then --五宝。兽决 银子40W
		积分 =278--380 看是否跳关
	elseif 名称 == "天火之殇下部" then--月华露、储灵袋、分解符、彩果、修炼果。c66 银子15W
		积分 =190
	elseif 名称 == "敦煌夜谭上部" then--月华露、储灵袋、分解符、彩果、修炼果。c66 银子15W
		积分 =150
	elseif 名称 == "敦煌夜谭下部" then--月华露、储灵袋、分解符、彩果、修炼果。c66 银子15W
		积分 =150
	elseif 名称 == "后羿陵墓下部" then--月华露、储灵袋、分解符、彩果、修炼果。c66 银子15W
		积分 =150
	elseif 名称 == "轮回镜" then
	end
