...
-- @Last Modified time: 2025-08-29 18:36:06
local 副本_七绝山 = class()
...
function 设置任务640(id)
	...
	for n=1,#队伍数据[队伍id].成员数据 do
		...
		if 副本数据.七绝山.完成[临时id]~=nil then
			...
			return
		elseif 玩家数据[临时id].角色:取任务(640)~=0 then
			常规提示(id,"#Y"..玩家数据[临时id].角色.名称.."正在进行副本任务，无法领取新的副本")
			return
		end
	end
	副本数据.七绝山.进行[id]={进程=7,数量=0}
...
__GWdh111[642]=function(连接id,数字id,序列,标识,地图)
	...
	if 副本数据.七绝山.进行[副本id].进程==2 and 副本数据.七绝山.进行[副本id].数量<15 then
		...
		if 取队伍人数(数字id)<5   then 常规提示(数字id,"#Y队伍人数低于3人，无法进入战斗") return  end
...
__GWdh111[647]=function(连接id,数字id,序列,标识,地图)
	...
	对话数据.模型=任务数据[标识].模型
  	对话数据.名称=任务数据[标识].名称
	local 副本id = 任务处理类:取副本id(数字id,640)
	if 对话数据.名称 == "山妖妖"  and  副本数据.七绝山.进行[副本id].数量 < 20 then
		对话数据.对话="目前还剩余"..(20-副本数据.七绝山.进行[副本id].数量).."个妖怪"
		对话数据.选项={"准备好了","饶你一命"}
	elseif 对话数据.名称 == "山怪怪"  and 副本数据.七绝山.进行[副本id].数量 < 20 then
		对话数据.对话="目前还剩余"..(20-副本数据.七绝山.进行[副本id].数量).."个妖怪"
		对话数据.选项={"准备好了","饶你一命"}
			...
			if  任务数据[任务id].类型== 640 then
				...
				for i=1,#队伍组 do
					if 玩家数据[队伍组[i]] ~= nil then
						玩家数据[队伍组[i]].角色:取消任务(玩家数据[队伍组[i]].角色:取任务(640))
						刷新队伍任务追踪(队伍组[i])
						副本数据.七绝山.完成[队伍组[i]] = true
						更新玩家每日(队伍组[i],"副本任务","七绝山")
						常规提示(队伍组[i], "很遗憾，副本失败了，请明天再来")
						任务处理类:副本结束传送(队伍组[i],5)
					end
				end
				任务数据[任务id]=nil
			else
...
function 任务说明640(玩家id,任务id)
	...
	if 副本数据.七绝山.进行[副本id]==nil then
		...
		elseif 进程==4 then
			说明={"七绝山",format("#L将大师分身带到星位协助大师布阵（完成度%s丨20）。(剩余时间%s分钟)",副本数据.七绝山.进行[副本id].数量,取分((任务数据[任务id].结束-(os.time()-任务数据[任务id].起始))))}
		elseif 进程==5 then
		...
		elseif 进程==7 then
			if 副本数据.七绝山.进行[副本id].数量 < 20 then
				说明={"七绝山",format("#L干掉山妖妖，山怪怪，抢夺食物，喂饱八戒（完成度%s丨20）。(剩余时间%s分钟)",副本数据.七绝山.进行[副本id].数量,取分((任务数据[任务id].结束-(os.time()-任务数据[任务id].起始))))}
			else
