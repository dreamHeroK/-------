...
function 副本_双城记:怪物对话内容(id,类型,标识,地图)
	...
	elseif 副本数据.如梦奇谭之双城记.进行[副本id].进程==4 then
		--if 副本数据.如梦奇谭之双城记.进行[副本id].阶段 == 0 then
			if 对话数据.名称=="黑衣人" then
...
function 副本_双城记:完成奖励(id组,战斗类型,倍率)
	local 倍率=倍率 or 1
	if  战斗类型 == 190301 then
		local 链接
		for n=1,#id组 do
			...
			玩家数据[cyid].角色:添加经验(经验*倍率,"双城记黑衣人")
			玩家数据[cyid].角色:添加银子(银子*倍率,"双城记黑衣人",1)
			if 取随机数()<=26 then
...
--					玩家数据[cyid].道具:给予超链接道具(cyid,名称,1,nil,链接)
	...
	elseif 战斗类型 == 190302 then
		for n=1,#id组 do
			local cyid=id组[n]
			local 等级=玩家数据[cyid].角色.等级
			local 经验=等级*取随机数(1500,1600)
			local 银子=等级*取随机数(100,120)+5000
			玩家数据[cyid].角色:添加经验(经验*倍率,"双城记木头人")
			玩家数据[cyid].角色:添加银子(银子*倍率,"双城记木头人",1)
			local 链接
			if 取随机数()<=26 then
				local 奖励参数=取随机数(1,100)
...
--					玩家数据[cyid].道具:给予超链接道具(cyid,名称,1,nil,链接)
	...
	elseif 战斗类型 == 190303 then
		for n=1,#id组 do
			local cyid=id组[n]
			local 等级=玩家数据[cyid].角色.等级
			local 经验=等级*取随机数(1500,1700)
			local 银子=等级*取随机数(100,120)+8000
			玩家数据[cyid].角色:添加经验(经验*倍率,"双城记武教头")
			玩家数据[cyid].角色:添加银子(银子*倍率,"双城记武教头",1)
			if 取随机数()<=50 then
				local 奖励参数=取随机数(1,100)
...
--					玩家数据[cyid].道具:给予超链接道具(cyid,名称,1,nil,链接)
	...
	elseif 战斗类型 == 190304 then
		for n=1,#id组 do
			...
			local 银子=等级*取随机数(100,120)+8000
			玩家数据[cyid].角色:添加经验(经验*倍率,"双城记凌云瀚")
			玩家数据[cyid].角色:添加银子(银子*倍率,"双城记凌云瀚",1)
			local 链接
			if 取随机数()<=26 then
				local 奖励参数=取随机数(1,100)
...
--					玩家数据[cyid].道具:给予超链接道具(cyid,名称,1,nil,链接)
	...
	elseif 战斗类型 == 190305 then
		for n=1,#id组 do
			...
			local 等级=玩家数据[cyid].角色.等级
			local 经验=等级*取随机数(1500,1600)
			local 银子=等级*取随机数(120,130)+6000
			玩家数据[cyid].角色:添加经验(经验*倍率,"双城记算筹子")
			玩家数据[cyid].角色:添加银子(银子*倍率,"双城记算筹子",1)
			local 链接
			if 取随机数()<=26 then
				local 奖励参数=取随机数(1,100)
...
--					玩家数据[cyid].道具:给予超链接道具(cyid,名称,1,nil,链接)
	...
	elseif 战斗类型 == 190306 then
		for n=1,#id组 do
			...
			local 银子=等级*取随机数(100,120)+8000
			玩家数据[cyid].角色:添加经验(经验*倍率,"双城记玄机之数")
			玩家数据[cyid].角色:添加银子(银子*倍率,"双城记玄机之数",1)
			local 链接
			if 取随机数()<=26 then
				local 奖励参数=取随机数(1,100)
...
--					玩家数据[cyid].道具:给予超链接道具(cyid,名称,1,nil,链接)
	...
	elseif 战斗类型 == 190307 then
		for n=1,#id组 do
			...
			玩家数据[cyid].角色:添加经验(经验*倍率,"双城记嗜血恶兽")
			玩家数据[cyid].角色:添加银子(银子*倍率,"双城记嗜血恶兽",1)
			local 链接
			if 取随机数()<=26 then
				local 奖励参数=取随机数(1,100)
...
--					玩家数据[cyid].道具:给予超链接道具(cyid,名称,1,nil,链接)
	...
	elseif 战斗类型 == 190308 then
		for n=1,#id组 do
			...
			local 经验=等级*取随机数(1500,1800)
			local 银子=等级*取随机数(100,120)+8000
			玩家数据[cyid].角色:添加经验(经验*倍率,"双城记邪影僵尸")
			玩家数据[cyid].角色:添加银子(银子*倍率,"双城记邪影僵尸",1)
			local 链接
			if 取随机数()<=26 then
				local 奖励参数=取随机数(1,100)
...
--					玩家数据[cyid].道具:给予超链接道具(cyid,名称,1,nil,链接)
	...
	elseif 战斗类型 == 190309 then
		...
		for n=1,#id组 do
			...
			local 经验=等级*取随机数(1500,1800)
			local 银子=等级*取随机数(90,120)+8000
			玩家数据[cyid].角色:添加经验(经验*倍率,"双城记宝库之门")
			玩家数据[cyid].角色:添加银子(银子*倍率,"双城记宝库之门",1)
...
function 副本_双城记:扫荡(队长id)
    ...
    local 队id = 玩家数据[队长id].队伍
    local 副本等级 = 50
    if not 队id or 队id == 0 or 玩家数据[队长id].队长 == false then
        常规提示(队长id, "#Y该任务必须组队完成且由队长领取")
        return
    elseif 取队伍人数(队长id) < 5 and 调试模式 == false then
        常规提示(队长id, "#Y此副本要求队伍人数不低于5人")
        return
    elseif 取等级要求(队长id, 副本等级) == false then
        常规提示(队长id, "#Y此副本要求角色等级不能低于"..副本等级.."级")
        return
    end
    local 成员列表 = 队伍数据[队id].成员数据
    for _, 临时id in ipairs(成员列表) do
        if 副本数据.如梦奇谭之双城记.完成[临时id] then
            常规提示(队长id, "#Y"..玩家数据[临时id].角色.名称.."本日已经完成过此副本了")
            return
        elseif 玩家数据[临时id].角色:取任务(800) ~= 0 then
            常规提示(队长id, "#Y"..玩家数据[临时id].角色.名称.."正在进行副本任务，无法加入")
            return
        elseif not 玩家数据[临时id].道具:判定背包道具(临时id, "癫散戏票·双城记", 1) then
            常规提示(队长id, "#Y"..玩家数据[临时id].角色.名称.."缺少“癫散戏票·双城记”，无法开启")
            常规提示(临时id, "#Y你缺少“癫散戏票·双城记”，无法参与副本")
            return
        end
    end
    for _, 临时id in ipairs(成员列表) do
    ...
    local 倍率=0.8
    local 战斗类型列表 = {190301, 190302, 190303, 190304, 190305, 190306, 190307, 190308, 190309}
    for _, 战型 in ipairs(战斗类型列表) do
        self:完成奖励(成员列表,战型,倍率)
    end
    for _, 临时id in ipairs(成员列表) do
        副本数据.如梦奇谭之双城记.完成[临时id] = true
        更新玩家每日(临时id, "副本任务", "如梦奇谭之双城记")
        常规提示(临时id, "#Y恭喜扫荡完成如梦奇谭之双城记，获得所有奖励！")
        if 玩家数据[临时id] and 玩家数据[临时id].连接id then
            发送数据(玩家数据[临时id].连接id, 550.3,{如梦奇谭之双城记=true})
        end
    end
end
