...
function 副本_天火之殇上:开启天火之殇上副本(id)
  ...
  elseif self:取天火之殇上次数限制(id) then
    return
  elseif 取队员任务存在(玩家数据[id].队伍,960) then
    常规提示(id,"#Y/队伍中已有队员正在进行该副本任,无法领取新的副本")
    return
  end
  local 任务id=id.."_960_"..os.time().."_"..随机序列.."_"..取随机数(1,999)
...
function 设置天火之殇上副本(id)
  ...
  if 副本数据.天火之殇上.进行[id].进程==1 then
    ...
    任务id=id.."_961_"..os.time().."_"..随机序列.."_"..取随机数(1,99)
    地图=6054
  ...
  elseif 副本数据.天火之殇上.进行[id].进程==3 then
    ...
    for i=1,2 do
      任务id=id.."_963_"..os.time().."_"..随机序列.."_"..取随机数(1,999)..i
      local 地图=6054
  ...
  elseif 副本数据.天火之殇上.进行[id].进程==4 then
    任务id=id.."_964_"..os.time().."_"..随机序列.."_"..取随机数(1,99)
    地图=6054
    local xy=地图处理类.地图坐标[地图]:取随机点()
    任务数据[任务id]={
      id=任务id,
  ...
  elseif 副本数据.天火之殇上.进行[id].进程==5 then
    ...
    任务数据[任务id]={
      ...
      y=125,
      方向=1,
      副本id=任务数据[玩家数据[id].角色:取任务(960)].副本id,
      地图编号=地图,
      地图名称=取地图名称(地图),
      类型=965,
    }
  ...
  elseif 副本数据.天火之殇上.进行[id].进程==6 then
    ...
    for i=1,8 do
      任务id=id.."_967_"..os.time().."_"..随机序列.."_"..取随机数(1,999)..i
      local 地图=6054
      local xy=地图处理类.地图坐标[地图]:取随机点()
      任务数据[任务id]={
        id=任务id,
  ...
  elseif 副本数据.天火之殇上.进行[id].进程==7 then
    ...
    地图处理类:添加单位(任务id)
    任务id=id.."_969_"..os.time().."_"..随机序列.."_"..取随机数(1,99)
    地图=6054
    local xy=地图处理类.地图坐标[地图]:取随机点()
    任务数据[任务id]={
      id=任务id,
  ...
  elseif 副本数据.天火之殇上.进行[id].进程==8 then
    任务id=id.."_970_"..os.time().."_"..随机序列.."_"..取随机数(1,99)
    地图=6055
    local xy=地图处理类.地图坐标[地图]:取随机点()
    任务数据[任务id]={
      id=任务id,
  ...
  elseif 副本数据.天火之殇上.进行[id].进程==9 then
    ...
    for i=1,2 do
      任务id=id.."_971_"..os.time().."_"..随机序列.."_"..取随机数(1,999)..i
      local 地图=6055
  ...
  elseif 副本数据.天火之殇上.进行[id].进程==10 then
    任务id=id.."_972_"..os.time().."_"..随机序列.."_"..取随机数(1,99)
    地图=6055
    local xy=地图处理类.地图坐标[地图]:取随机点()
    任务数据[任务id]={
      id=任务id,
  ...
  elseif 副本数据.天火之殇上.进行[id].进程==11 then
    任务id=id.."_973_"..os.time().."_"..随机序列.."_"..取随机数(1,99)
    地图=6055
    local xy=地图处理类.地图坐标[地图]:取随机点()
    任务数据[任务id]={
      id=任务id,
  ...
  elseif 副本数据.天火之殇上.进行[id].进程==12 then
    ...
    for i=1,2 do
      任务id=id.."_974_"..os.time().."_"..随机序列.."_"..取随机数(1,999)..i
      local 地图=6055
  ...
  elseif 副本数据.天火之殇上.进行[id].进程==13 then
    任务id=id.."_975_"..os.time().."_"..随机序列.."_"..取随机数(1,99)
    地图=6056
    local xy=地图处理类.地图坐标[地图]:取随机点()
    任务数据[任务id]={
      id=任务id,
  ...
  elseif 副本数据.天火之殇上.进行[id].进程==14 then
    ...
    for i=1,2 do
      任务id=id.."_976_"..os.time().."_"..随机序列.."_"..取随机数(1,999)..i
      local 地图=6056
      local xy=地图处理类.地图坐标[地图]:取随机点()
      任务数据[任务id]={
        id=任务id,
  ...
  elseif 副本数据.天火之殇上.进行[id].进程==15 then
    任务id=id.."_977_"..os.time().."_"..随机序列.."_"..取随机数(1,99)
    地图=6056
    local xy=地图处理类.地图坐标[地图]:取随机点()
    任务数据[任务id]={
      id=任务id,
    ...
    地图处理类:添加单位(任务id)
    任务id=id.."_978_"..os.time().."_"..随机序列.."_"..取随机数(1,99)
    地图=6057
    local xy=地图处理类.地图坐标[地图]:取随机点()
    任务数据[任务id]={
      id=任务id,
...
-----------------------------------------结束--------------------------------------------------
function 天火之殇拘魂镜(id)
  if 玩家数据[id].角色:取任务(966) ~= 0 then
    添加最后对话(id,"少侠身上已经任务，请先完成任务再来！")
    return
  end
  local 地图=6054
  local xy=地图处理类.地图坐标[地图]:取随机点()
  local 任务id="_966_"..os.time().."_"..随机序列.."_"..取随机数(1,9999)
  任务数据[任务id]={
    id=任务id,
    起始=os.time(),
    结束=1200,
...
__GWdh111[961]=function(连接id,数字id,序列,标识,地图)
  ...
  if 任务数据[标识].zhandou==nil then
    ...
    if (任务数据[玩家数据[数字id].角色:取任务(960)] or {}).副本id ~= 副本id then 添加最后对话(数字id,"我好像不认识你吧？？？") return end
    任务数据[标识].zhandou=true
    战斗准备类:创建战斗(数字id+0,155599,标识) ---鬼魂
...
__GWdh111[963]=function(连接id,数字id,序列,标识,地图)
  ...
  if 任务数据[标识].zhandou==nil then
    ...
    if (任务数据[玩家数据[数字id].角色:取任务(960)] or {}).副本id ~= 副本id then 添加最后对话(数字id,"我好像不认识你吧？？？") return end
    任务数据[标识].zhandou=true
    战斗准备类:创建战斗(数字id+0,155600,标识) ---魑魅
...
__GWdh111[964]=function(连接id,数字id,序列,标识,地图)
  ...
  对话数据.名称=任务数据[标识].名称
    ...
    if (任务数据[玩家数据[数字id].角色:取任务(960)] or {}).副本id ~= 副本id then 添加最后对话(数字id,"我好像不认识你吧？？？") return end
      ...
      完成天火之殇上任务({数字id},玩家数据[数字id].地图单位.标识,964.1)
    end
  return 对话数据
end
...
__GWdh222[965]=function (连接id,数字id,序号,内容)
  ...
  if 事件 == "我要一个拘魂镜" then
    ...
    if 任务数据[玩家数据[数字id].地图单位.标识].拘魂镜领取 then return end
    玩家数据[数字id].道具:给予道具(数字id,"拘魂镜")
    常规提示(数字id,"#Y/你得到了拘魂镜")
    任务数据[玩家数据[数字id].地图单位.标识].拘魂镜领取 = true
    天火之殇拘魂镜(数字id)
    玩家数据[数字id].地图单位=nil
    return
  end
...
__GWdh111[967]=function(连接id,数字id,序列,标识,地图)
  ...
  if 任务数据[标识].zhandou==nil then
    ...
    if (任务数据[玩家数据[数字id].角色:取任务(960)] or {}).副本id ~= 副本id then 添加最后对话(数字id,"我好像不认识你吧？？？") return end
    任务数据[标识].zhandou=true
    战斗准备类:创建战斗(数字id+0,155601,标识) ---鬼魂士兵
...
__GWdh222[968]=function (连接id,数字id,序号,内容)
  local 事件=内容[1]
  local 名称=内容[3]
  if 事件=="竟然如此？那我这就去查看！" then
    完成天火之殇上任务({数字id},玩家数据[数字id].地图单位.标识,968.1)
  end
end
__GWdh111[970]=function(连接id,数字id,序列,标识,地图)
  local 对话数据 = {}
  对话数据.模型=任务数据[标识].模型
  对话数据.名称=任务数据[标识].名称
  对话数据.选项={}
  ...
  if 副本数据.天火之殇上.进行[数字id].进程==8 then
    ...
    local 副本id=任务数据[标识].副本id
    if (任务数据[玩家数据[数字id].角色:取任务(960)] or {}).副本id ~= 副本id then 添加最后对话(数字id,"我好像不认识你吧？？？") return end
      完成天火之殇上任务({数字id},玩家数据[数字id].地图单位.标识,970.1)
    end
    end
  return 对话数据
end
__GWdh111[971]=function(连接id,数字id,序列,标识,地图)
  local 对话数据={}
  对话数据.模型=任务数据[标识].模型
  对话数据.名称=任务数据[标识].名称
  ...
  if 任务数据[标识].zhandou==nil then
    ...
    if (任务数据[玩家数据[数字id].角色:取任务(960)] or {}).副本id ~= 副本id then 添加最后对话(数字id,"我好像不认识你吧？？？") return end
    任务数据[标识].zhandou=true
    战斗准备类:创建战斗(数字id+0,155602,标识) ---巡逻鬼
    玩家数据[数字id].地图单位=nil
    return
  else
    对话数据.对话="我正在战斗中，请勿打扰。"
...
__GWdh111[972]=function(连接id,数字id,序列,标识,地图)
  local 对话数据 = {}
    对话数据.模型=任务数据[标识].模型
  对话数据.名称=任务数据[标识].名称
  if 任务数据[标识].zhandou==nil then
...
__GWdh222[972]=function (连接id,数字id,序号,内容)
  local 事件=内容[1]
  local 名称=内容[3]
  if 事件=="我来乔装成巡正鬼首领" then
    if 任务数据[玩家数据[数字id].地图单位.标识]==nil then return end
    完成天火之殇上任务({数字id},玩家数据[数字id].地图单位.标识,972.1)
    玩家数据[数字id].地图单位=nil
    return
  end
end
__GWdh111[973]=function(连接id,数字id,序列,标识,地图)
  local 对话数据 = {}
    对话数据.模型=任务数据[标识].模型
  对话数据.名称=任务数据[标识].名称
  if 任务数据[标识].zhandou==nil then
...
__GWdh222[973]=function (连接id,数字id,序号,内容)
  local 事件=内容[1]
  local 名称=内容[3]
  if 事件=="想逃？！没门！" then
    if 任务数据[玩家数据[数字id].地图单位.标识]==nil then return end
    ...
    战斗准备类:创建战斗(数字id+0,155603,玩家数据[数字id].地图单位.标识)  ---鬼魂头目
    任务数据[玩家数据[数字id].地图单位.标识].zhandou=true
    玩家数据[数字id].地图单位=nil
    return
  end
end
__GWdh111[974]=function(连接id,数字id,序列,标识,地图)
  local 对话数据 = {}
  对话数据.模型=任务数据[标识].模型
  对话数据.名称=任务数据[标识].名称
    local 副本id=任务数据[标识].副本id
...
__GWdh222[974]=function (连接id,数字id,序号,内容)
  local 事件=内容[1]
  local 名称=内容[3]
  if 事件=="我看你是找打！" then
    if 任务数据[玩家数据[数字id].地图单位.标识]==nil then return end
    ...
    if 取队伍人数(数字id)<1 then 常规提示(数字id,"#Y/挑战天火之殇最少要有五人") return  end
    if 取等级要求(数字id,60)==false then 常规提示(数字id,"#Y/挑战天火之殇至少要达到60级") return  end
    战斗准备类:创建战斗(数字id+0,155604,玩家数据[数字id].地图单位.标识)  ---韩刘首领[
    任务数据[玩家数据[数字id].地图单位.标识].zhandou=true
    玩家数据[数字id].地图单位=nil
    return
  elseif 事件=="先铲除你!" then
    ...
    任务数据[玩家数据[数字id].地图单位.标识].zhandou=true  --崔判官
    玩家数据[数字id].地图单位=nil
    return
  end
end
__GWdh111[975]=function(连接id,数字id,序列,标识,地图)
  local 对话数据 = {}
    对话数据.模型=任务数据[标识].模型
  对话数据.名称=任务数据[标识].名称
  if 任务数据[标识].zhandou==nil then
...
__GWdh222[975]=function (连接id,数字id,序号,内容)
  local 事件=内容[1]
  local 名称=内容[3]
  if 事件=="我还是比较喜欢打架，请让我干掉那些护卫去！" then
    if 任务数据[玩家数据[数字id].地图单位.标识]==nil then return end
    完成天火之殇上任务({数字id},玩家数据[数字id].地图单位.标识,975.1)
    玩家数据[数字id].地图单位=nil
    return
  end
end
__GWdh111[976]=function(连接id,数字id,序列,标识,地图)
  local 对话数据={}
  对话数据.模型=任务数据[标识].模型
  对话数据.名称=任务数据[标识].名称
  if 任务数据[标识].zhandou==nil then
    ...
    if (任务数据[玩家数据[数字id].角色:取任务(960)] or {}).副本id ~= 副本id then 添加最后对话(数字id,"我好像不认识你吧？？？") return end
    任务数据[标识].zhandou=true
    战斗准备类:创建战斗(数字id+0,155606,标识) ---鬼魂护卫
    对话数据.对话 = "#24男人你们也要抢!这些壮丁是要献给皇上壮大军队用的!不会让给你们!"
    玩家数据[数字id].地图单位=nil
    return
  else
...
__GWdh111[977]=function(连接id,数字id,序列,标识,地图)--
  local 对话数据 = {}
  对话数据.模型=任务数据[标识].模型
  对话数据.名称=任务数据[标识].名称
  if 任务数据[标识].zhandou==nil then
...
__GWdh222[977]=function (连接id,数字id,序号,内容)
  local 事件=内容[1]
  local 名称=内容[3]
...
__GWdh111[978]=function(连接id,数字id,序列,标识,地图)
  local 对话数据={}
  对话数据.模型=任务数据[标识].模型
  对话数据.名称=任务数据[标识].名称
  ...
  if 任务数据[标识].zhandou==nil then
    ...
    任务数据[标识].zhandou=true
    战斗准备类:创建战斗(数字id+0,155607,标识) ---隋炀帝鬼魂
    --玩家数据[数字id].地图单位=nil
    return
  else
    对话数据.对话="我正在战斗中，请勿打扰。"
...
function 完成天火之殇上任务(id组,任务id,战斗类型,扫荡,倍率)
  local 奖励
  local 倍率=倍率 or 1
  if 任务数据[任务id] then
    ...
    for i=1,#id组 do
      ...
      local 银子
      local 储备
    if 战斗类型==155599 then --鬼魂
    local 等级=玩家数据[id].角色.等级
    local 经验=等级*取随机数(10000,12000)
    local 银子=等级*取随机数(90,120)+34000
    玩家数据[id].角色:添加经验(经验*倍率,"鬼魂")
    玩家数据[id].角色:添加银子(银子*倍率,"鬼魂",1)
    玩家数据[id].zhandou=0
    if 取随机数()<=25 then
      ...
      elseif 战斗类型==155600 then --魑魅
    local 等级=玩家数据[id].角色.等级
    local 经验=等级*取随机数(12000,14000)
    local 银子=等级*取随机数(90,120)+54000
    玩家数据[id].角色:添加经验(经验*倍率,"魑魅")
    玩家数据[id].角色:添加银子(银子*倍率,"魑魅",1)
    玩家数据[id].zhandou=0
    if 取随机数()<=25 then
      local 奖励参数=取随机数(1,100)
      链接={提示=format("#S(副本-天火之殇上部)#G/%s#Y/在天火之殇上部中发现一个物品，立马一脚踩住，心想‘嘿嘿，没人看到吧’！#G",玩家数据[id].角色.名称),频道="xt",结尾="#Y。"}
   ...
   elseif 战斗类型==966 then --拘魂境
    local 等级=玩家数据[id].角色.等级
    local 经验=等级*取随机数(14000,16000)
    local 银子=等级*取随机数(90,120)+64000
    玩家数据[id].角色:添加经验(经验*倍率,"拘魂境")
    玩家数据[id].角色:添加银子(银子*倍率,"拘魂境",1)
    玩家数据[id].zhandou=0
    if 取随机数()<=25 then
      local 奖励参数=取随机数(1,100)
      链接={提示=format("#S(副本-天火之殇上部)#G/%s#Y/在天火之殇上部中发现一个物品，立马一脚踩住，心想‘嘿嘿，没人看到吧’！#G",玩家数据[id].角色.名称),频道="xt",结尾="#Y。"}
 ...
 elseif 战斗类型==155601 then --鬼魂士兵
    local 等级=玩家数据[id].角色.等级
    local 经验=等级*取随机数(12000,13000)
    local 银子=等级*取随机数(90,120)+34000
    玩家数据[id].角色:添加经验(经验*倍率,"鬼魂士兵")
    玩家数据[id].角色:添加银子(银子*倍率,"鬼魂士兵",1)
    玩家数据[id].zhandou=0
    if 取随机数()<=25 then
      local 奖励参数=取随机数(1,100)
      链接={提示=format("#S(副本-天火之殇上部)#G/%s#Y/在天火之殇上部中发现一个物品，立马一脚踩住，心想‘嘿嘿，没人看到吧’！#G",玩家数据[id].角色.名称),频道="xt",结尾="#Y。"}
     ...
     elseif 战斗类型==155602 then --巡逻鬼
    local 等级=玩家数据[id].角色.等级
    local 经验=等级*取随机数(14000,17000)
    local 银子=等级*取随机数(90,120)+74000
    玩家数据[id].角色:添加经验(经验*倍率,"巡逻鬼")
    玩家数据[id].角色:添加银子(银子*倍率,"巡逻鬼",1)
    玩家数据[id].zhandou=0
    if 取随机数()<=25 then
      local 奖励参数=取随机数(1,100)
      链接={提示=format("#S(副本-天火之殇上部)#G/%s#Y/在天火之殇上部中发现一个物品，立马一脚踩住，心想‘嘿嘿，没人看到吧’！#G",玩家数据[id].角色.名称),频道="xt",结尾="#Y。"}
      ...
      elseif 战斗类型==155603 then --鬼魂头目
    local 等级=玩家数据[id].角色.等级
    local 经验=等级*取随机数(14000,17000)
    local 银子=等级*取随机数(90,120)+74000
    玩家数据[id].角色:添加经验(经验*倍率,"鬼魂头目")
    玩家数据[id].角色:添加银子(银子*倍率,"鬼魂头目",1)
    玩家数据[id].zhandou=0
    if 取随机数()<=25 then
      local 奖励参数=取随机数(1,100)
      链接={提示=format("#S(副本-天火之殇上部)#G/%s#Y/在天火之殇上部中发现一个物品，立马一脚踩住，心想‘嘿嘿，没人看到吧’！#G",玩家数据[id].角色.名称),频道="xt",结尾="#Y。"}
      ...
      elseif 奖励参数<=75 then
        ...
        玩家数据[id].道具:给予超链接道具(id,名称,取随机数(1,1),nil,链接)
          ...
          elseif 战斗类型==155604 then --韩擒虎
    local 等级=玩家数据[id].角色.等级
    local 经验=等级*取随机数(14000,17000)
    local 银子=等级*取随机数(90,120)+74000
    玩家数据[id].角色:添加经验(经验*倍率,"韩擒虎")
    玩家数据[id].角色:添加银子(银子*倍率,"韩擒虎",1)
    玩家数据[id].zhandou=0
    if 取随机数()<=25 then
      local 奖励参数=取随机数(1,100)
      链接={提示=format("#S(副本-天火之殇上部)#G/%s#Y/在天火之殇上部中发现一个物品，立马一脚踩住，心想‘嘿嘿，没人看到吧’！#G",玩家数据[id].角色.名称),频道="xt",结尾="#Y。"}
    ...
    elseif 战斗类型==155605 then --刘武周
    local 等级=玩家数据[id].角色.等级
    local 经验=等级*取随机数(14000,17000)
    local 银子=等级*取随机数(90,120)+74000
    玩家数据[id].角色:添加经验(经验*倍率,"刘武周")
    玩家数据[id].角色:添加银子(银子*倍率,"刘武周",1)
    玩家数据[id].zhandou=0
    if 取随机数()<=25 then
      local 奖励参数=取随机数(1,100)
      链接={提示=format("#S(副本-天火之殇上部)#G/%s#Y/在天火之殇上部中发现一个物品，立马一脚踩住，心想‘嘿嘿，没人看到吧’！#G",玩家数据[id].角色.名称),频道="xt",结尾="#Y。"}
    ...
    elseif 战斗类型==155606 then --鬼魂护卫
    local 等级=玩家数据[id].角色.等级
    local 经验=等级*取随机数(12000,14000)
    local 银子=等级*取随机数(90,120)+54000
    玩家数据[id].角色:添加经验(经验*倍率,"鬼魂护卫")
    玩家数据[id].角色:添加银子(银子*倍率,"鬼魂护卫",1)
    玩家数据[id].zhandou=0
    if 取随机数()<=25 then
      local 奖励参数=取随机数(1,100)
      链接={提示=format("#S(副本-天火之殇上部)#G/%s#Y/在天火之殇上部中发现一个物品，立马一脚踩住，心想‘嘿嘿，没人看到吧’！#G",玩家数据[id].角色.名称),频道="xt",结尾="#Y。"}
    ...
    elseif 战斗类型==155607 then --隋炀帝鬼魂
    local 等级=玩家数据[id].角色.等级
    local 经验=等级*取随机数(17000,19000)
    local 银子=等级*取随机数(90,120)+140000
    玩家数据[id].角色:添加经验(经验*倍率,"隋炀帝鬼魂")
    玩家数据[id].角色:添加银子(银子*倍率,"隋炀帝鬼魂",1)
    玩家数据[id].zhandou=0
    if 取随机数()<=25 then
      local 奖励参数=取随机数(1,100)
      链接={提示=format("#S(副本-天火之殇上部)#G/%s#Y/在天火之殇上部中发现一个物品，立马一脚踩住，心想‘嘿嘿，没人看到吧’！#G",玩家数据[id].角色.名称),频道="xt",结尾="#Y。"}
...
if not 扫荡 then
    ...
    elseif 战斗类型==962.1 then
        ...
        设置天火之殇上副本(副本id)
        进度刷新=true
    elseif 战斗类型==964.1 then
        副本数据.天火之殇上.进行[副本id].进程=5
        设置天火之殇上副本(副本id)
        进度刷新=true
    elseif 战斗类型==966 then
       if 玩家数据[id组[1]].角色:取任务(960)  then
        取消队伍任务(玩家数据[id组[1]].角色:取任务(960),966)
    ...
    elseif 战斗类型==968.1 then
        副本数据.天火之殇上.进行[副本id].进程=8
        设置天火之殇上副本(副本id)
        进度刷新=true
        任务处理类:副本传送(id组[1],16)
    elseif 战斗类型==970.1 then
   ...
   elseif 战斗类型==155604 then
        if 玩家数据[id组[1]].角色:取任务(960) and 副本数据.天火之殇上.进行[副本id].韩刘.韩擒虎 < 1 then
   ...
   elseif 战斗类型==155605 then
        if 玩家数据[id组[1]].角色:取任务(960) and 副本数据.天火之殇上.进行[副本id].韩刘.刘武周 < 1 then
...
function 结束天火之殇上副本(任务id)
  ...
  for i=1,#id组 do
    if 玩家数据[id组[i]] then
      玩家数据[id组[i]].角色:取消任务(960)
      玩家数据[id组[i]].角色:刷新任务跟踪()
      副本数据.天火之殇上.完成[id组[i]] = true
      if 玩家数据[id组[i]].战斗 ~= 0 and ((战斗准备类.战斗盒子[玩家数据[id组[i]].战斗] or {}).战斗类型==155599 or (战斗准备类.战斗盒子[玩家数据[id组[i]].战斗] or {}).战斗类型==155600 or (战斗准备类.战斗盒子[玩家数据[id组[i]].战斗] or {}).战斗类型==155601 or (战斗准备类.战斗盒子[玩家数据[id组[i]].战斗] or {}).战斗类型==155602 or (战斗准备类.战斗盒子[玩家数据[id组[i]].战斗] or {}).战斗类型==155603 or (战斗准备类.战斗盒子[玩家数据[id组[i]].战斗] or {}).战斗类型==155604 or (战斗准备类.战斗盒子[玩家数据[id组[i]].战斗] or {}).战斗类型==155605 or (战斗准备类.战斗盒子[玩家数据[id组[i]].战斗] or {}).战斗类型==155606 or (战斗准备类.战斗盒子[玩家数据[id组[i]].战斗] or {}).战斗类型==155607) then
...
function 天火之殇上副本失败(任务id)
  if os.time()-任务数据[任务id].起始>=任务数据[任务id].结束 and 任务数据[任务id].结束 ~= 99999999 then
    if 任务数据[任务id].zhandou~=true then  --未进入战斗状态
      if  任务数据[任务id].类型== 960 then
        for i=1,#任务数据[任务id].队伍组 do
          if 玩家数据[任务数据[任务id].队伍组[i]] ~= nil then
            副本数据.天火之殇上.完成[任务数据[任务id].队伍组[i]] = true
            更新玩家每日(任务数据[任务id].队伍组[i],"副本任务","天火之殇上")
            玩家数据[任务数据[任务id].队伍组[i]].角色:取消任务(玩家数据[任务数据[任务id].队伍组[i]].角色:取任务(960))
          end
        end
        任务数据[任务id]=nil
      else
...
function 副本_天火之殇上:扫荡(队长id)
    ...
    local 最低等级 = 60
    local 非调试最小人数 = 5
    -- 所有需发放奖励的类型（覆盖副本全阶段：战斗+对话进度）
    local 全奖励类型 = {
        155599,155600,962.1,964.1,966,155601,
        968.1,970.1,155602,972.1,155603,155604,
        155605,975.1,155606,155607
    }
    -- 2. 合规校验（防异常，保留核心门槛）
    local 队伍id = 玩家数据[队长id].队伍
    if  队伍id ==0 then 常规提示(队长id,"#Y请先组队！"); return end
    local 队员列表 = 队伍数据[队伍id].成员数据
    -- 2.1 人数/等级/副本次数校验
    if #队员列表 < 非调试最小人数 and 调试模式 == false then
        常规提示(队长id,"#Y需5人队伍！"); return
    end
    for _,id in ipairs(队员列表) do
        if 取等级要求(id,最低等级) == false then
            常规提示(队长id,"#Y"..玩家数据[id].角色.名称.."等级不足"..最低等级.."级！"); return
        end
        if 副本数据.天火之殇上.完成[id] or 玩家数据[id].角色:取任务(960) ~= 0 then
            常规提示(队长id,"#Y"..玩家数据[id].角色.名称.."已完成/在其他副本中！"); return
        end
    end
    -- 3. 最小化副本状态初始化（仅支撑奖励发放，不创建实际单位）
    local 副本标识id = 队长id
    local 主任务id = "temp_skyfire_main_"..副本标识id.."_"..os.time()
    -- 初始化奖励函数必需的进度状态（跳过中间步骤需直接满足条件）
    副本数据.天火之殇上.进行[副本标识id] = {
        进程 = 15, -- 直接拉满至最终进程
        主任务id = 主任务id,
        数量 = 2, -- 满足鬼魂解救进度要求
        魑魅 = 2, -- 满足魑魅击杀进度要求
    ...
    -- 4. 核心：批量发放全阶段奖励（复用原奖励逻辑，无中间步骤）
    for _,奖励类型 in ipairs(全奖励类型) do
        -- 创建临时任务id（奖励函数需任务id参数，用完即删）
        local 临时任务id = "temp_skyfire_award_"..奖励类型.."_"..os.time()
        任务数据[临时任务id] = {
            id = 临时任务id,
            副本id = 副本标识id,
            名称 = 奖励类型==155604 and "韩擒虎鬼魂" or (奖励类型==155605 and "刘武周幻影" or "临时单位"),
            地图编号 = 6054 -- 随意填，不影响奖励发放
        }
        -- 调用原函数发放奖励（自动触发经验、银子、道具奖励）
        完成天火之殇上任务(队员列表, 临时任务id, 奖励类型,true,倍率)
        -- 即时清理临时数据，避免残留
        任务数据[临时任务id] = nil
    end
    -- 5. 收尾：标记副本完成+清理状态（不影响原副本逻辑）
    for _,id in ipairs(队员列表) do
        if 玩家数据[id] then
            -- 标记副本完成+更新每日进度
            副本数据.天火之殇上.完成[id] = true
            更新玩家每日(id,"副本任务","天火之殇上部")
            -- 取消主任务+刷新任务面板
            玩家数据[id].角色:取消任务(960)
            玩家数据[id].角色:刷新任务跟踪()
            -- 发送完成提示
            常规提示(id,"#Y天火之殇上部副本已直接完成，所有奖励已发放！")
            if 玩家数据[id] and 玩家数据[id].连接id then
                  发送数据(玩家数据[id].连接id, 550.3,{天火之殇上=true})
            end
        end
    end
    -- 清理副本临时状态
    副本数据.天火之殇上.进行[副本标识id] = nil
    任务数据[主任务id] = nil
    -- 常规提示(队长id,"#Y所有队员已完成天火之殇上部副本，奖励发放完毕！")
end
