      ...
      if  任务数据[任务id].类型== 70 then
        for i=1,#任务数据[任务id].队伍组 do
          if 玩家数据[任务数据[任务id].队伍组[i]] ~= nil then
            ...
            地图处理类:跳转地图(任务数据[任务id].队伍组[i],1001,363,34)
          end
        end
        任务数据[任务id]=nil
      else
...
function 完成大闹天宫任务(id组,任务id,战斗类型,倍率,扫荡)
  local 奖励
  local 倍率=倍率 or 1
  if 任务数据[任务id] then
    local 副本id=任务数据[任务id].副本id
   ...
   local 等级=玩家数据[id].角色.等级
    ...
    玩家数据[id].角色:添加经验(经验*倍率,"大闹天宫")
    玩家数据[id].角色:添加银子(银子*倍率,"大闹天宫",1)
    玩家数据[id].zhandou=0
    if 取随机数()<=25 then
...
function 副本_大闹天宫:扫荡(队长id)
    ...
    local 倍率 = 0.8
    local 副本等级 = 60 -- 基础等级门槛
    local 最小人数 = 5 -- 非调试模式最小队伍人数
    local 所有奖励类型 ={
     71,          -- 大闹锄树
        72,          -- 大闹浇水
        155585,      -- 大闹修桃
        ...
        155581,      -- 天兵天将
        155582,      -- 二郎真君
        155583       -- 最终BOSS（含特效宝珠奖励）
      }
    local 队伍id = 玩家数据[队长id].队伍
    if not 队伍id  or 队伍id==0 then 常规提示(队长id,"#Y请先组队！") return end
    local 成员列表 = 队伍数据[队伍id].成员数据
    if #成员列表 < 最小人数 and 调试模式==false then
        常规提示(队长id,"#Y需5人队伍！")
        return
    end
    for _,id in ipairs(成员列表) do
        if 取等级要求(id,副本等级)==false then
            常规提示(队长id,"#Y"..玩家数据[id].角色.名称.."等级不足60级！"); return
        end
        if 副本数据.大闹天宫.完成[id] or 玩家数据[id].角色:取任务(70)~=0 then
            常规提示(队长id,"#Y"..玩家数据[id].角色.名称.."已完成/在其他副本中！"); return
        end
    end
    -- 3. 初始化副本状态（支撑奖励发放，不触发进程切换）
    local 副本标识id = 队长id
    local 主任务id = 副本标识id.."_temp_70_"..os.time()
    -- 初始化奖励所需状态（三大力士/战诸神等，确保奖励能正常触发）
    副本数据.大闹天宫.进行[副本标识id] = {
        主任务id = 主任务id,
        三大力士 = {锄树力士=1,浇水力士=1,修桃力士=1},
        战诸神 = {造酒仙官=false,运水道人=false,烧火童子=false,盘槽力士=false},
        数量 = 5, -- 适配蟠桃/小猴奖励判断
        天兵天将 = {天兵=false,天将=false}
    }
    -- 初始化主任务数据（满足奖励函数参数要求）
    任务数据[主任务id] = {
        id = 主任务id,
    ...
    for _,奖类型 in ipairs(所有奖励类型) do
        local 临时任务id = "temp_"..副本标识id.."_"..奖类型
        任务数据[临时任务id] = {
            id = 临时任务id,
            副本id = 副本标识id,
            地图编号 = 6033, -- 随便填，不影响发奖
            单位编号 = 1000,
            名称 = 奖类型==155588 and "二郎真君" or "临时单位" -- 适配部分奖励名称判断
        }
        完成大闹天宫任务(成员列表, 临时任务id, 奖类型,倍率,true)
        任务数据[临时任务id] = nil
    end
    for _,id in ipairs(成员列表) do
        if 玩家数据[id] then
            副本数据.大闹天宫.完成[id] = true -- 标记当日完成
            更新玩家每日(id,"副本任务","大闹天宫") -- 更新每日进度
            常规提示(id,"#Y大闹天宫所有进程已完成，奖励已发放！")
            if 玩家数据[id] and 玩家数据[id].连接id then
              发送数据(玩家数据[id].连接id, 550.3,{大闹天宫=true})
            end
        end
    end
    副本数据.大闹天宫.进行[副本标识id] = nil
    任务数据[主任务id] = nil
end
