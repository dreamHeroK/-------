...
-- @最后修改来自: baidwwy
-- @Last Modified time: 2023-05-24 01:35:28
local 副本_一斛珠 = class()
local sj = 取随机数
...
function 刷新任务690(id)
	...
	if 副本数据.如梦奇谭之一斛珠.进行[id].进程==1 then
		--if 副本数据.如梦奇谭之一斛珠.进行[id].阶段 == 0 then
		local 地图=7006 --四方城
		...
		for n=1,#队伍数据[队伍id].成员数据 do
			local 临时id=队伍数据[队伍id].成员数据[n]
			发送数据(玩家数据[临时id].连接id,6557,{文本=text1,
				类型="一斛珠序",字体=nil,音乐=nil,背景=nil,横排显示=nil,动画调用=nil})
		end
		--elseif 副本数据.如梦奇谭之一斛珠.进行[id].阶段 == 1 then
			--常规提示(id,"#Y/此处为播放动画")
			--return
		--end
	elseif 副本数据.如梦奇谭之一斛珠.进行[id].进程==2 then
	 ...
	 --  	对话数据.选项={}
		...
		for n=1,#队伍数据[队伍id].成员数据 do
			local 临时id=队伍数据[队伍id].成员数据[n]
			发送数据(玩家数据[临时id].连接id,6557,{文本=text1,
				类型="一斛珠1",字体=nil,音乐=nil,背景=nil,横排显示=true,动画调用=nil})
		end
	...
	elseif 副本数据.如梦奇谭之一斛珠.进行[id].进程==7 then
			...
			for n=1,#队伍数据[队伍id].成员数据 do
				local 临时id=队伍数据[队伍id].成员数据[n]
				发送数据(玩家数据[临时id].连接id,6557,{文本=text1,
					类型="一斛珠2",字体=nil,音乐=nil,背景=nil,横排显示=true,动画调用=nil})
			end
			发送数据(玩家数据[id].连接id,6557,{文本=text1,
			类型="五更寒渔村2",字体=nil,音乐=nil,背景=nil,横排显示=true,动画={100,5}})
	...
	elseif 副本数据.如梦奇谭之一斛珠.进行[id].进程==9 then
			...
			for n=1,#队伍数据[队伍id].成员数据 do
				local 临时id=队伍数据[队伍id].成员数据[n]
				发送数据(玩家数据[临时id].连接id,6557,{文本=text1,
					类型="一斛珠3",字体=nil,音乐=nil,背景=nil,横排显示=true,动画=nil})
			end
	...
	elseif 副本数据.如梦奇谭之一斛珠.进行[id].进程==11 then
		...
		for n=1,#队伍数据[队伍id].成员数据 do
			...
			更新玩家每日(临时id,"副本任务","如梦奇谭之一斛珠")
			发送数据(玩家数据[临时id].连接id,6557,{文本=text1,
				类型="一斛珠结束",字体=nil,音乐=nil,背景=nil,横排显示=nil,动画=nil})
		end
	end
end
...
function 副本_一斛珠:怪物对话内容(id,类型,标识,地图)
	...
	elseif 副本数据.如梦奇谭之一斛珠.进行[副本id].进程==3 then
		--if 副本数据.如梦奇谭之一斛珠.进行[副本id].阶段 == 0 then
			if 对话数据.名称=="叶夫人" then
...
function 副本_一斛珠:完成奖励(id组,战斗类型,倍率)
	local 倍率=倍率 or 1
	if 战斗类型 == 190101 then
		for n=1,#id组 do
			local cyid=id组[n]
			local 等级=玩家数据[cyid].角色.等级
			local 经验=等级*取随机数(1650,2100)
			local 银子=等级*取随机数(110,120)+9000
			玩家数据[cyid].角色:添加经验(经验*倍率,"一斛珠鸾儿")
			玩家数据[cyid].角色:添加银子(银子*倍率,"一斛珠鸾儿",1)
...
--					玩家数据[cyid].道具:给予超链接道具(cyid,名称,1,nil,链接)
	...
	elseif 战斗类型 == 190102 then
		for n=1,#id组 do
			local cyid=id组[n]
			local 等级=玩家数据[cyid].角色.等级
			local 经验=等级*取随机数(1650,2050)
			local 银子=等级*取随机数(110,120)+8000
			玩家数据[cyid].角色:添加经验(经验*倍率,"一斛珠叶夫人")
			玩家数据[cyid].角色:添加银子(银子*倍率,"一斛珠叶夫人",1)
...
--					玩家数据[cyid].道具:给予超链接道具(cyid,名称,1,nil,链接)
	...
	elseif 战斗类型 == 190103 then
		for n=1,#id组 do
			...
			local 经验=等级*取随机数(1650,2050)
			local 银子=等级*取随机数(110,120)+8000
			玩家数据[cyid].角色:添加经验(经验*倍率,"一斛珠官兵")
			玩家数据[cyid].角色:添加银子(银子*倍率,"一斛珠官兵",1)
...
--					玩家数据[cyid].道具:给予超链接道具(cyid,名称,1,nil,链接)
	...
	elseif 战斗类型 == 190104 then
		for n=1,#id组 do
			...
			玩家数据[cyid].角色:添加经验(经验*倍率,"一斛珠新郎沈唐")
			玩家数据[cyid].角色:添加银子(银子*倍率,"一斛珠新郎沈唐",1)
...
--					玩家数据[cyid].道具:给予超链接道具(cyid,名称,1,nil,链接)
	...
	elseif 战斗类型 == 190105 then
		for n=1,#id组 do
			...
			玩家数据[cyid].角色:添加经验(经验*倍率,"一斛珠夜影迷踪")
			玩家数据[cyid].角色:添加银子(银子*倍率,"一斛珠夜影迷踪",1)
...
--					玩家数据[cyid].道具:给予超链接道具(cyid,名称,1,nil,链接)
	...
	elseif 战斗类型 == 190106 then
		...
		for n=1,#id组 do
			local cyid=id组[n]
			local 等级=玩家数据[cyid].角色.等级
			local 经验=等级*取随机数(1800,2100)
			local 银子=等级*取随机数(120,130)+10000
			玩家数据[cyid].角色:添加经验(经验*倍率,"一斛珠完成奖励")
			玩家数据[cyid].角色:添加银子(银子*倍率,"一斛珠完成奖励",1)
...
function 副本_一斛珠:扫荡(队长id)
    ...
    local 副本id = 队长id -- 沿用原逻辑：用队长id作为副本标识
    local 副本等级 = 50 -- 原副本等级要求
    for _, 临时id in ipairs(队伍成员列表) do
        if 副本数据.如梦奇谭之一斛珠.完成[临时id] then
            常规提示(队长id, "#Y"..玩家数据[临时id].角色.名称.."本日已完成此副本，无法重复参与！")
            return
        elseif 玩家数据[临时id].角色:取任务(690) ~= 0 then
            常规提示(队长id, "#Y"..玩家数据[临时id].角色.名称.."正在进行其他副本，无法加入！")
            return
        elseif not 玩家数据[临时id].道具:判定背包道具(临时id,"癫散戏票·一斛珠",1) then
            常规提示(队长id, "#Y"..玩家数据[临时id].角色.名称.."缺少“癫散戏票·一斛珠”，无法开启！")
            return
        elseif 取等级要求(临时id, 副本等级) == false then
            常规提示(队长id, "#Y"..玩家数据[临时id].角色.名称.."等级低于"..副本等级.."级，无法参与！")
            return
        end
    end
    for _, 临时id in ipairs(队伍成员列表) do
    ...
    local 所有战斗类型 = {190101, 190102, 190103, 190104, 190105, 190106} -- 原副本所有战斗类型
    ...
    for _, 临时id in ipairs(队伍成员列表) do
        副本数据.如梦奇谭之一斛珠.完成[临时id] = true
        更新玩家每日(临时id, "副本任务", "如梦奇谭之一斛珠")
        常规提示(临时id, "#Y恭喜扫荡完成如梦奇谭之一斛珠副本，获得所有奖励！")
    end
    for _, 临时id in ipairs(队伍成员列表) do
        if 玩家数据[临时id] and 玩家数据[临时id].连接id then
            发送数据(玩家数据[临时id].连接id, 550.3,{如梦奇谭之一斛珠=true})
        end
    end
