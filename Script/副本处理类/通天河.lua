...
-- @最后修改来自: baidwwy
-- @Last Modified time: 2023-05-24 01:36:49
local 副本_通天河 = class()
local sj = 取随机数
...
function 副本_通天河:开启副本(id)
	...
	if 玩家数据[id].队伍==0 or 玩家数据[id].队长==false  then
		...
		return
	elseif 取队伍人数(id)<5 and not 调试模式 then
...
function 副本_通天河:对话事件处理(id,名称,事件,类型)
	...
	elseif 类型 == 712 and 名称=="灵灯" and  事件=="伸手触摸一下" then
		if 副本数据.通天河.进行[副本id].进程==2 then
			...
			if 取队伍人数(id)<1  then 常规提示(id,"#Y队伍人数低于1人，无法进入战斗") return  end
	...
	elseif 类型 == 718 and 副本数据.通天河.进行[副本id].进程 == 8 then
		...
		if 事件=="杀" then
			...
			if ls[名称] ~= nil then
				if 任务数据[玩家数据[id].地图单位.标识].zhandou~=nil then 常规提示(id,"#Y/对方正在战斗中") return  end
				if 取队伍人数(id)<1  then 常规提示(id,"#Y队伍人数低于3人，无法进入战斗") return  end
				任务数据[玩家数据[id].地图单位.标识].zhandou=true
...
function 副本_通天河:完成奖励(id组,战斗类型,倍率)
	local  倍率 = 倍率 or 1
	if 战斗类型 == 190201 then
		local 链接
		for n=1,#id组 do
			...
			玩家数据[cyid].角色:添加经验(经验*倍率,"灵感分身")
			玩家数据[cyid].角色:添加银子(银子*倍率,"灵感分身",1)
			local 链接
			if 取随机数()<=20 then
	...
	elseif 战斗类型 == 190202 then
		for n=1,#id组 do
			...
			local 经验=等级*取随机数(1500,1800)
			local 银子=等级*取随机数(100,120)+8000
			玩家数据[cyid].角色:添加经验(经验*倍率,"通天河红孩儿")
			玩家数据[cyid].角色:添加银子(银子*倍率,"通天河红孩儿",1)
			local 链接
			if 取随机数()<=26 then
				local 奖励参数=取随机数(1,100)
	...
	elseif 战斗类型 == 190203 then
		for n=1,#id组 do
			...
			local 等级=玩家数据[cyid].角色.等级
			local 经验=等级*取随机数(1500,1800)
			local 银子=等级*取随机数(100,130)+8000
			玩家数据[cyid].角色:添加经验(经验*倍率,"通天河木材")
			玩家数据[cyid].角色:添加银子(银子*倍率,"通天河木材",1)
			local 链接
			if 取随机数()<=26 then
				local 奖励参数=取随机数(1,100)
	...
	elseif 战斗类型 == 190208 then
		for n=1,#id组 do
			...
			玩家数据[cyid].角色:添加经验(经验*倍率,"通天河金鱼")
			玩家数据[cyid].角色:添加银子(银子*倍率,"通天河金鱼",1)
			local 链接
			if 取随机数()<=26 then
				local 奖励参数=取随机数(1,100)
	...
	elseif 战斗类型 == 190209 then
		for n=1,#id组 do
			...
			local 等级=玩家数据[cyid].角色.等级
			local 经验=等级*取随机数(1500,1800)
			local 银子=等级*取随机数(120,150)+8000
			玩家数据[cyid].角色:添加经验(经验*倍率,"通天河婴灵")
			玩家数据[cyid].角色:添加银子(银子*倍率,"通天河婴灵",1)
			local 链接
			if 取随机数()<=26 then
				local 奖励参数=取随机数(1,100)
	...
	elseif 战斗类型 == 190210 then
		...
		-- stid=id组[stid]
		-- local 链接={提示=format("#S(通天河)#G/%s#R超度了冤魂，获得了",玩家数据[stid].角色.名称),频道="xt",结尾="#R！#80"}
		-- 玩家数据[stid].道具:给予超链接书铁(stid,{15,15},nil,链接)
		for n=1,#id组 do
			local cyid=id组[n]
			local 等级=玩家数据[cyid].角色.等级
			local 经验=等级*取随机数(1500,1800)
			local 银子=等级*取随机数(120,150)+10000
			玩家数据[cyid].角色:添加经验(经验*倍率,"通天河冤魂")
			玩家数据[cyid].角色:添加银子(银子*倍率,"通天河冤魂",1)
	...
	elseif 战斗类型 == 190211 then
		for n=1,#id组 do
			...
			local 经验=等级*取随机数(1500,1800)
			local 银子=等级*取随机数(120,150)+10000
			玩家数据[cyid].角色:添加经验(经验*倍率,"通天河心")
			玩家数据[cyid].角色:添加银子(银子*倍率,"通天河心",1)
			local 链接
			if 取随机数()<=26 then
				local 奖励参数=取随机数(1,100)
	...
	elseif 战斗类型 == 190212 then
		for n=1,#id组 do
			...
			local 经验=等级*取随机数(1500,1800)
			local 银子=等级*取随机数(120,150)+10000
			玩家数据[cyid].角色:添加经验(经验*倍率,"通天河肝")
			玩家数据[cyid].角色:添加银子(银子*倍率,"通天河肝",1)
			local 链接
			if 取随机数()<=26 then
				local 奖励参数=取随机数(1,100)
	...
	elseif 战斗类型 == 190213 then
		for n=1,#id组 do
			...
			local 经验=等级*取随机数(1500,1800)
			local 银子=等级*取随机数(120,150)+10000
			玩家数据[cyid].角色:添加经验(经验*倍率,"通天河肾")
			玩家数据[cyid].角色:添加银子(银子*倍率,"通天河肾",1)
			local 链接
			if 取随机数()<=26 then
				local 奖励参数=取随机数(1,100)
	...
	elseif 战斗类型 == 190214 then
		for n=1,#id组 do
			...
			local 经验=等级*取随机数(1500,1800)
			local 银子=等级*取随机数(120,150)+10000
			玩家数据[cyid].角色:添加经验(经验*倍率,"通天河脾")
			玩家数据[cyid].角色:添加银子(银子*倍率,"通天河脾",1)
			local 链接
			if 取随机数()<=26 then
				local 奖励参数=取随机数(1,100)
	...
	elseif 战斗类型 == 190215 then
		for n=1,#id组 do
			...
			local 经验=等级*取随机数(1500,1800)
			local 银子=等级*取随机数(120,150)+10000
			玩家数据[cyid].角色:添加经验(经验*倍率,"通天河肺")
			玩家数据[cyid].角色:添加银子(银子*倍率,"通天河肺",1)
			local 链接
			if 取随机数()<=26 then
				local 奖励参数=取随机数(1,100)
	...
	elseif 战斗类型 == 190216 then
		...
		for n=1,#id组 do
			...
			玩家数据[cyid].角色:添加经验(经验*倍率,"通天河灵感大王")
			玩家数据[cyid].角色:添加银子(银子*倍率,"通天河灵感大王",1)
			local 链接
			if 取随机数()<=26 then
				local 奖励参数=取随机数(1,100)
...
function 副本_通天河:扫荡(队长id)
    ...
    local 副本id = 队长id
    local 主任务id, ZU = 取唯一任务(710, 副本id) -- 复用原任务ID生成逻辑

    -- 初始化副本进程（直接设为最终进程10，跳过中间环节）
    副本数据.通天河.进行[副本id] = {
        进程 = 10,
        答题 = {}, -- 空表避免后续函数报错
        怪物 = {}  -- 空表避免后续函数报错
    ...
    for _, 临时id in ipairs(队伍成员列表) do
        ...
        if 玩家数据[临时id] and 玩家数据[临时id].连接id then
            发送数据(玩家数据[临时id].连接id, 550.3,{通天河=true})
        end
        常规提示(临时id, "#Y恭喜！已直接完成通天河副本，所有奖励已发放！")
    end
end
