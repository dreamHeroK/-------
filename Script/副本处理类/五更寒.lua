...
function 胜利MOB_190001(id组,战斗类型,任务id,倍率) --五更寒慧明长老
	local 倍率=倍率 or 1
	...
	刷新任务670(副本id)
	刷新队伍任务追踪(id组[1])
	玩家数据[id组[1]].道具:给予道具(id组[1],"竹木签",1,nil,"不存共享")
	local 链接
	for n=1,#id组 do
		local cyid=id组[n]
		local 等级=玩家数据[cyid].角色.等级
		local 经验=等级*取随机数(1950,2200)
		local 银子=等级*取随机数(110,130)+18000
		玩家数据[cyid].角色:添加经验(经验*倍率,"五更寒慧明长老")
		玩家数据[cyid].角色:添加银子(银子*倍率,"五更寒慧明长老",1)
		local 链接
		if 取随机数()<=20 then
...
function 胜利MOB_190002(id组,战斗类型,任务id,倍率) --五更寒小鱼儿
	任务数据[任务id].zhandou = nil
	local 副本id=任务数据[任务id].副本id
	local 倍率=倍率 or 1
	--任务处理类:删除单位(任务id)
	...
	for n=1,#id组 do
		local cyid=id组[n]
		local 等级=玩家数据[cyid].角色.等级
		local 经验=等级*取随机数(2250,2500)
		local 银子=等级*取随机数(120,140)+18000
		玩家数据[cyid].角色:添加经验(经验*倍率,"五更寒小鱼儿")
		玩家数据[cyid].角色:添加银子(银子*倍率,"五更寒小鱼儿",1)
		local 链接
		if 取随机数()<=26 then
			local 奖励参数=取随机数(1,100)
...
function 胜利MOB_190003(id组,战斗类型,任务id,倍率) --五更寒魔化村民赌徒
	任务数据[任务id].zhandou = nil
	local 副本id=任务数据[任务id].副本id
	local 倍率=倍率 or 1
	--任务处理类:删除单位(任务id)
...
function 胜利MOB_190004(id组,战斗类型,任务id,倍率) --五更寒魔化村民山贼
	任务数据[任务id].zhandou = nil
	local 副本id=任务数据[任务id].副本id
	local 倍率=倍率 or 1
	--任务处理类:删除单位(任务id)
	...
	for n=1,#id组 do
		local cyid=id组[n]
		local 等级=玩家数据[cyid].角色.等级
		local 经验=等级*取随机数(1800,2000)
		local 银子=等级*取随机数(120,140)+18000
		玩家数据[cyid].角色:添加经验(经验*倍率,"五更寒魔化村民山贼")
		玩家数据[cyid].角色:添加银子(银子*倍率,"五更寒魔化村民山贼",1)
		local 链接
		if 取随机数()<=26 then
			local 奖励参数=取随机数(1,100)
			链接={提示=format("#S(奇谭-五更寒)#G/%s#Y正义凛然，打的魔化村民落荒而逃，并缴获",玩家数据[cyid].角色.名称),频道="xt",结尾="#Y一个！"}
...
function 胜利MOB_190005(id组,战斗类型,任务id,倍率) --五更寒魔化村民夜叉
	任务数据[任务id].zhandou = nil
	local 副本id=任务数据[任务id].副本id
	local 倍率=倍率 or 1
	--任务处理类:删除单位(任务id)
	...
	for n=1,#id组 do
		local cyid=id组[n]
		local 等级=玩家数据[cyid].角色.等级
		local 经验=等级*取随机数(1950,2100)
		local 银子=等级*取随机数(100,120)+18000
		玩家数据[cyid].角色:添加经验(经验*倍率,"五更寒魔化村民山贼")
		玩家数据[cyid].角色:添加银子(银子*倍率,"五更寒魔化村民山贼",1)
		local 链接
		if 取随机数()<=26 then
...
function 胜利MOB_190006(id组,战斗类型,任务id,倍率) --五更寒魔化村长
	任务数据[任务id].zhandou = nil
	local 副本id=任务数据[任务id].副本id
	local 倍率 =倍率 or 1
	...
	玩家数据[stid].道具:给予超链接道具(stid,"神兜兜",5,nil,链接)
	for n=1,#id组 do
		local cyid=id组[n]
		local 等级=玩家数据[cyid].角色.等级
		local 经验=等级*取随机数(1900,2100)
		local 银子=等级*取随机数(100,120)+20000
		玩家数据[cyid].角色:添加经验(经验*倍率,"五更寒魔化村长")
		玩家数据[cyid].角色:添加银子(银子*倍率,"五更寒魔化村长",1)
		if 取随机数()<=40 then
			local 奖励参数=取随机数(1,100)
...
function 胜利MOB_190007(id组,战斗类型,任务id,倍率) --五更寒八足海妖
	任务数据[任务id].zhandou = nil
	local 副本id=任务数据[任务id].副本id
	local 倍率=倍率 or 1
	...
	for n=1,#id组 do
		...
		local 经验=等级*取随机数(1900,2100)
		local 银子=等级*取随机数(100,120)+20000
		玩家数据[cyid].角色:添加经验(经验*倍率,"五更寒八足海妖")
		玩家数据[cyid].角色:添加银子(银子*倍率,"五更寒八足海妖",1)
...
-- ".."#R/qqq|魔化村长*7005*临时npc*52*33/魔化村长".." 潘大嫂
...
function 副本_五更寒:扫荡(队长id)
    ...
    local 副本id = 队长id -- 用队长id作为副本标识

    -- 校验成员状态：是否已完成、是否在副本中、是否有戏票
    for _, 临时id in ipairs(队伍成员列表) do
        if 副本数据.如梦奇谭之五更寒.完成[临时id] then
            常规提示(队长id, "#Y"..玩家数据[临时id].角色.名称.."本日已完成此副本，无法重复参与！")
            return
        elseif 玩家数据[临时id].角色:取任务(670) ~= 0 then
            常规提示(队长id, "#Y"..玩家数据[临时id].角色.名称.."正在进行其他副本，无法加入！")
            return
        elseif not 玩家数据[临时id].道具:判定背包道具(临时id,"癫散戏票·五更寒",1) then
            常规提示(队长id, "#Y"..玩家数据[临时id].角色.名称.."缺少“癫散戏票·五更寒”，无法开启！")
            return
        end
    end
    ...
    -- 初始化副本进程（仅保留必要的任务数据，支撑奖励函数）
    local 主任务id, ZU = 取唯一任务(670, 队长id)
    任务数据[主任务id] = {
        id=主任务id, 队伍组=ZU, 副本id=副本id, 类型=670 -- 仅保留奖励函数依赖的字段
    }
    副本数据.如梦奇谭之五更寒.进行[副本id] = {进程=1} -- 初始化进程，避免奖励函数报错
    -- 进程1：慧明长老战斗奖励
    local 临时任务id1 = 副本id.."_671_temp"
    任务数据[临时任务id1] = {副本id=副本id, 类型=671}
    胜利MOB_190001(队伍成员列表, 190001, 临时任务id1,倍率) -- 发放该进程奖励
    任务数据[临时任务id1] = nil
    -- 进程7→8：小鱼儿战斗奖励
    local 临时任务id2 = 副本id.."_677_temp"
    任务数据[临时任务id2] = {副本id=副本id, 类型=677}
    胜利MOB_190002(队伍成员列表, 190002, 临时任务id2,倍率) -- 发放该进程奖励
    任务数据[临时任务id2] = nil

    -- 进程8→9：魔化村民（赌徒）战斗奖励
    local 临时任务id3 = 副本id.."_678_temp"
    任务数据[临时任务id3] = {副本id=副本id, 类型=678}
    胜利MOB_190003(队伍成员列表, 190003, 临时任务id3,倍率) -- 发放该进程奖励
    任务数据[临时任务id3] = nil

    -- 进程9阶段1：魔化村民（山贼）战斗奖励
    local 临时任务id4 = 副本id.."_679_1_temp"
    任务数据[临时任务id4] = {副本id=副本id, 类型=679}
    胜利MOB_190004(队伍成员列表, 190004, 临时任务id4,倍率) -- 发放该进程奖励
    任务数据[临时任务id4] = nil

    -- 进程9阶段2：魔化村民（夜叉）战斗奖励
    local 临时任务id5 = 副本id.."_679_2_temp"
    任务数据[临时任务id5] = {副本id=副本id, 类型=679}
    胜利MOB_190005(队伍成员列表, 190005, 临时任务id5,倍率) -- 发放该进程奖励
    任务数据[临时任务id5] = nil

    -- 进程10→11：魔化村长战斗奖励
    local 临时任务id6 = 副本id.."_680_temp"
    任务数据[临时任务id6] = {副本id=副本id, 类型=680}
    胜利MOB_190006(队伍成员列表, 190006, 临时任务id6,倍率) -- 发放该进程奖励
    任务数据[临时任务id6] = nil

    -- 进程11→12：八足海妖战斗奖励
    local 临时任务id7 = 副本id.."_681_temp"
    任务数据[临时任务id7] = {副本id=副本id, 类型=681}
    胜利MOB_190007(队伍成员列表, 190007, 临时任务id7,倍率) -- 发放该进程奖励
    任务数据[临时任务id7] = nil
    -- --------------------------
    -- 副本结束：标记完成+同步客户端状态
    -- --------------------------
    ...
    for _, 临时id in ipairs(队伍成员列表) do
        副本数据.如梦奇谭之五更寒.完成[临时id] = true
        更新玩家每日(临时id, "副本任务", "如梦奇谭之五更寒") -- 记录每日完成状态
        常规提示(临时id, "#Y恭喜扫荡完成五更寒副本，获得所有奖励！") -- 简洁提示
    end

    -- 清理副本数据
    副本数据.如梦奇谭之五更寒.进行[副本id] = nil
    任务数据[主任务id] = nil

    for _, 临时id in ipairs(队伍成员列表) do
        if 玩家数据[临时id] and 玩家数据[临时id].连接id then
            发送数据(玩家数据[临时id].连接id, 550.3,{如梦奇谭之五更寒=true})
        end
    end
end
